for(let i=0; i<sessionStorage.length; i++){
    if(sessionStorage.key(i).startsWith('sb-') && sessionStorage.key(i).endsWith('-auth-token')){
      ...
      break;
            ...
            }, 5000);async function submitLandingForm(e) {
                e.preventDefault();
    ...
    function defaultDB() {
      return {
        ...
        admissions: [],
        classes: [
          { id: 'c_mont', name: 'Montessori', section: 'A', level: 'primary', capacity: 25, teacher: '' },
          { id: 'c_nur', name: 'Nursery', section: 'A', level: 'primary', capacity: 25, teacher: '' },
          { id: 'c_prep', name: 'Prep', section: 'A', level: 'primary', capacity: 30, teacher: '' },
          ...
          { id: 'c11', name: 'First Year (11th)', section: 'Pre-Eng', level: 'secondary', capacity: 50, teacher: '' },
          { id: 'c12', name: 'Second Year (12th)', section: 'Pre-Eng', level: 'secondary', capacity: 50, teacher: '' },
        ],
        subjects: [
    ...
    function getClsSubjects(cid) { const cls = (DB.classes || []).find(c => c.id === cid); if (cls && cls.subjectIds && Array.isArray(cls.subjectIds) && cls.subjectIds.length) { const list = (DB.subjects || []).filter(s => cls.subjectIds.includes(s.id)); if (list.length) return list; } return DB.subjects || []; }
    ...
    function updateSOM() {
      const som = DB?.settings?.studentOfTheMonth;
      if (som) {
        const nameEl = document.getElementById('som-name');
        const gradeEl = document.getElementById('som-grade');
        const descEl = document.getElementById('som-desc');
        const imgEl = document.getElementById('som-img');
        if (nameEl) nameEl.textContent = som.name;
        if (gradeEl) gradeEl.textContent = som.grade;
        if (descEl) descEl.textContent = som.description;
        if (som.photo && imgEl) imgEl.src = som.photo;
      }
    }
    function makeStudentOfMonth(sid, cid) {
      const s = clsStus(cid).find(x => x.id === sid);
      const cls = DB.classes.find(c => c.id === cid);
      if (!s || !cls) return;
      const desc = prompt(`Why is ${s.name} the Student of the Month?`, `Has shown exceptional dedication and outstanding academic achievement.`);
      if (desc === null) return;
      if (!DB.settings) DB.settings = {};
      DB.settings.studentOfTheMonth = { name: s.name, grade: cls.name, description: desc, photo: s.photo || '' };
      saveDB();
      updateSOM();
      showToast(`${s.name} is now Student of the Month!`, 'ts');
      closeModal();
    }

    function filterTable(input, tableId) {
      const q = input.value.toLowerCase().trim();
      const tbody = tableId ? document.getElementById(tableId) : input.closest('.card')?.querySelector('tbody');
      if (!tbody) return;
    ...
    function monthlyFeeAmount(stu) {
      const fee = (DB.fees.items || []).find(f => f.id === 'f1') || { amount: 5000 };
      if (!stu) return num(fee.amount || 5000);
      const cf = num(stu.customFee);
      if (cf > 0) return cf;
      return num(fee.amount || 5000);
    }
    function ensureMonthlyFeeLedger() {
      ...
      const cm = monthKey();
      allStus().forEach(s => {
        if (s.customFee === 0 || s.customFee === '0') s.customFee = '';
        const sid = s.id;
        DB.fees.monthlyLedger[sid] = DB.fees.monthlyLedger[sid] || {};
        const activeFee = monthlyFeeAmount(s);
        if (!DB.fees.monthlyLedger[sid][cm]) {
          DB.fees.monthlyLedger[sid][cm] = { month: cm, paid: false, amount: activeFee };
        } else if (!DB.fees.monthlyLedger[sid][cm].paid) {
          DB.fees.monthlyLedger[sid][cm].amount = activeFee;
        }
    ...
    function ensureEnhancements() {
      ...
      // Cleanup orphaned students (e.g. from test admissions with invalid class IDs)
      const validClassIds = new Set((DB.classes || []).map(c => c.id));
      if (DB.students) {
        Object.keys(DB.students).forEach(cid => {
          if (!validClassIds.has(cid)) {
            // Remove from DB.users as well
            const orphanedIds = DB.students[cid].map(s => s.id);
            if (DB.users && orphanedIds.length) {
              DB.users = DB.users.filter(u => !(u.role === 'student' && orphanedIds.includes(u.studentId)));
            }
            delete DB.students[cid];
          }
        });
      }
    ...
    async function doForgotPass() {
      const loginValue = (document.getElementById('l-email').value || '').trim().toLowerCase();
      const em = resolvePortalLoginEmail(loginValue);
      if (!em) { showToast('Please enter your email first', 'tw2'); return; }
      if (!_sb) { showToast('Supabase Auth is not configured', 'te'); return; }
      const btn = document.querySelector('.login-btn');
      ...
      try {
        ...
        if (error) throw error;
        showToast('Reset link sent to ' + em + '!', 'ts', 5000);
      } catch (e) {
        showToast(e.message || 'Error sending reset email', 'te', 5000);
      }
      if (btn) btn.textContent = 'Portal Sign In';
    }
    ...
    async function doSignUp() {
      ...
      const em = resolvePortalLoginEmail(loginValue);
      const pw = (document.getElementById('l-pass').value || '').trim();
      if (!em || !pw) { showToast('Enter your assigned email and a new password', 'tw2', 5000); return; }
      if (pw.length < 6) { showToast('Password must be at least 6 characters', 'tw2'); return; }
      
      const u = findUserProfile(em, loginRole);
      if (!u) {
        showToast(`No ${loginRole} account found for this email. Contact your administrator to be added first.`, 'te', 6000);
        return;
      }
      if (!_sb) { showToast('Supabase Auth is not configured', 'te'); return; }
      ...
      try {
        const { data, error } = await _sb.auth.signUp({ email: em, password: pw });
        if (error) {
           if (error.message.includes('User already registered') || error.message.includes('already exists')) {
              showToast('Account already exists! Try signing in, or use Forgot Password.', 'tw2', 5000);
           } else {
              showToast(error.message, 'te', 5000);
           }
           return;
        }
        if (data && data.user && !data.session) {
           showToast('Account created! Please check your email inbox to confirm your email before logging in.', 'tw2', 8000);
           console.warn('NOTE: If you want to disable email confirmations, go to Supabase Dashboard -> Authentication -> Providers -> Email and turn off "Confirm email".');
        } else {
           showToast('Password set successfully! You can now log in.', 'ts', 5000);
        }
      } finally {
        if (btn) { btn.disabled = false; btn.textContent = 'Portal Sign In'; }
      }
    ...
    function applyBranding() {
      ...
      ['tb-ico', 'lp-logo', 'land-mark', 'foot-logo'].forEach(id => {
        ...
        el.innerHTML = s.logo ? `<img src="${s.logo}" alt="logo" style="width:100%;height:100%;object-fit:contain;"/>` : `<img src="favicon.png" alt="logo" style="width:100%;height:100%;object-fit:contain;"/>`;
    ...
    function navItems() {
      ...
      if (curRole === 'teacher') return [
        ...
        { p: 'notices', i: '', l: 'Notices & Assignments', s: 'News' },
        { p: 'mytimetable', i: '', l: 'My Timetable', s: 'Sched' },
        { p: 'mypayroll', i: '', l: 'My Payslips', s: 'Payslip' },
      ];
      return [
        { p: 'dashboard', i: 'dashboard', l: 'Dashboard', s: 'Home' },
  ...
  <button class="sb-out" onclick="logout()"><span class="ni-ic">${iconMarkup('logout')}</span> Sign Out</button>`;
    ...
    function goPage(p) {
      ...
      if (p === 'idcards') { setTimeout(buildClassIdCardQrs, 50); setTimeout(buildClassIdCardQrs, 150); setTimeout(buildClassIdCardQrs, 350); }
    ...
    function renderPage(p) {
      const map = {
        ...
        mygrades: pgMyGrades, myattend: pgMyAttend, qrcard: pgQRCard, myfees: pgMyFees, profile: pgProfile, mypayroll: pgMyPayroll,
        mytimetable: pgMyTimetable
      };
  ...
  <div>
    ...
    <div class="card fu d3">
      <div class="ch"><div class="ct"> Teacher Payroll</div><button class="btn btn-gh btn-sm" onclick="goPage('payroll')">Open</button></div>
        ${teachers.slice(0, 3).map(t => `<div style="display:flex;justify-content:space-between;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);"><div><div style="font-weight:800;font-size:.8rem;">${t.name}</div><div style="font-size:.68rem;color:var(--ink4);">${t.subject || 'General'} - ${t.payrollStatus === 'paid' && t.lastPaid ? `Paid ${fmtDate(t.lastPaid)}` : 'Awaiting salary release'}</div></div><div style="text-align:right;"><div style="font-weight:800;color:var(--p);">${fmtMoney(calcPayrollData(t, monthKey()).expectedNet)}</div><span class="badge ${t.payrollStatus === 'paid' ? 'bg2' : 'bo2'}">${t.payrollStatus === 'paid' ? 'Paid' : 'Pending'}</span></div></div>`).join('') || '<div style="font-size:.76rem;color:var(--ink4);">No teachers added yet</div>'}
    </div>
...
<div class="g2">
  <div class="card fu d2">
    ...
    <div class="score-mini">${getClsSubjects(stu.classId || curCls).map(sub => { const v = (DB.grades[cid + '_' + curTerm]?.[stu.id] || {})[sub.id] || 0; return `<div class="sm-row"><div class="sm-lbl">${sub.icon} ${sub.name}</div><div class="sm-bar"><div class="sm-fill" style="width:${v}%;background:${sub.color};"></div></div><div class="sm-val" style="color:${sub.color};">${v || '-'}</div></div>`; }).join('')}</div>
  </div>
  <div>
    ...
    <div class="card fu d5" style="margin-top:12px;">
      ...
      <div style="padding:10px 0; display:flex; flex-wrap:wrap; gap:8px;">
        ${(DB.users || []).filter(u => u.role === 'teacher' && (u.classIds || []).includes(cid)).map(t => `<a href="mailto:${t.email}?subject=Message from ${stu.name} (Parent/Student)" class="btn btn-gh btn-xs" style="text-decoration:none;"><span style="margin-right:4px;"><span class="material-symbols-outlined" style="font-size:inherit;vertical-align:-2px;">mail</span></span>${t.name}</a>`).join('') || '<div style="color:var(--ink4);font-size:.76rem;">No teachers assigned</div>'}
      </div>
    </div>
  </div>
</div>
...
<div class="ph-acts">
    <input type="text" class="fi" placeholder="Search student or roll..." onkeyup="filterTable(this, 'students-tbody')" style="width:200px; padding:6px 10px; margin-right:4px;"/>
    <button class="btn btn-gh btn-sm" onclick="switchIdCardClass('${curCls}')">Print IDs</button>
    ${curRole !== 'teacher' ? `<button class="btn btn-gh btn-sm" onclick="openBulkModal()"> Bulk Import</button>
  ...
  <div class="tw">
    <table>
      ...
      <tbody id="students-tbody">
          ${sts.length === 0 ? `<tr><td colspan="10" style="text-align:center;padding:28px;"><div style="color:var(--ink4);margin-bottom:10px;">No students yet</div>${curRole !== 'teacher' ? `<button class="btn btn-p btn-sm" onclick="openAddStudentModal()">+ Add First Student</button>` : ''}</td></tr>` : ''}
          ${sts.map((s, i) => {
        const av = avgStu(curCls, s.id); const ap = attPct(curCls, s.id); const fam = s.familyId ? DB.families[s.familyId] : null; return `<tr>
          ...
          <td><div style="display:flex;gap:3px;flex-wrap:wrap;">
            ...
            ${curRole !== 'teacher' ? `<button class="btn btn-p btn-xs" onclick="openAddStudentModal('${s.id}')">Edit</button>
            <button class="btn btn-xs" style="background:var(--gl);color:var(--g);" onclick="printAdmissionForm('${s.id}','${curCls}')" title="Print Admission Form"><span class="material-symbols-outlined" style="font-size:inherit;vertical-align:-2px;">assignment</span> Adm.</button>
...
<div class="card fu">
  <div class="ch">
    <div class="ct">Applications</div>
    <input type="text" class="fi" placeholder="Search applicant..." onkeyup="filterTable(this, 'admissions-tbody')" style="width:200px; padding:6px 10px; margin-left:auto; margin-right:8px;"/>
    <button class="btn btn-gh btn-sm" onclick="refreshAdmissionApplications()">Refresh Online</button>
  </div>
    ${apps.length ? `<div class="tbl-wrap"><table><thead><tr><th>Student</th><th>Class</th><th>Parent</th><th>Phone</th><th>Date</th><th>Status</th><th>Actions</th></tr></thead><tbody id="admissions-tbody">${apps.map(a => `<tr><td><b>${escHtml(a.studentName)}</b><div style="font-size:.68rem;color:var(--ink4);">${escHtml(a.gender || '')}${a.dob ? ' - DOB ' + fmtDate(a.dob) : ''}</div></td><td>${escHtml(a.className || '-')}</td><td>${escHtml(a.father || '-')}<div style="font-size:.68rem;color:var(--ink4);">${escHtml(a.email || '')}</div></td><td>${escHtml(a.phone || '-')}</td><td>${fmtDate(a.createdAt)}</td><td><span class="badge ${admissionStatusClass(a.status)}">${escHtml(a.status || 'new')}</span></td><td><div style="display:flex;gap:6px;flex-wrap:wrap;"><button class="btn btn-gh btn-xs" onclick="openAdmissionDetail('${a.id}')">View</button><a href="mailto:${a.email}?subject=Regarding Admission for ${a.studentName}" class="btn btn-gh btn-xs" onclick="contactAdmission('${a.id}')">Email</a><button class="btn btn-g btn-xs" onclick="approveAdmission('${a.id}')">Approve</button><button class="btn btn-r btn-xs" onclick="deleteAdmission('${a.id}')">Delete</button></div></td></tr>`).join('')}</tbody></table></div>` : '<div style="text-align:center;color:var(--ink4);padding:24px;">No admission applications yet.</div>'}
</div>`;
    }
    ...
    async function deleteAdmission(id) {
      ...
      saveDB();
      if (_sb) { try { await _sb.from('admission_applications').delete().eq('id', id); } catch (e) {} }
      goPage('admissions');
      showToast('Application deleted', 'ts');
    }
  ...
  <div class="fg2">
    <div class="fg"><label class="fl">Full Name <span class="req">*</span></label><input class="fi" id="m-name" value="${s?.name || ''}" placeholder="Arsalan"/></div>
  ...
  <div class="fg2">
    <div class="fg"><label class="fl">Father's Name</label><input class="fi" id="m-father" value="${s?.father || ''}" placeholder="Mr"/></div>
  ...
  <div class="fg2">
    <div class="fg"><label class="fl">Monthly Fee</label><input class="fi" type="number" id="m-customfee" value="${s ? monthlyFeeAmount(s) : ''}" placeholder="Use default fee"/></div>
    <div class="fg"><label class="fl">Van Fee (Transport)</label><input class="fi" type="number" id="m-vanfee" value="${(s && s.vanFee !== undefined && s.vanFee !== '') ? num(s.vanFee) : ''}" placeholder="0"/></div>
  </div>
  <div class="fg2">
    ...
    <div class="fg"><label class="fl">Address</label><textarea class="fi" id="m-addr" style="min-height:44px;">${s?.address || ''}</textarea></div>
  </div>
  <div class="fg2">
  ...
  <div class="m-btns">
    ...
    async function saveStu(editId) {
      ...
      const vfVal = document.getElementById('m-vanfee')?.value;
      const arrVal = document.getElementById('m-arrears')?.value;
      const data = { name, roll, classId, gender: document.getElementById('m-gender')?.value || '', dob: document.getElementById('m-dob')?.value || '', blood: document.getElementById('m-blood')?.value || '', religion: (document.getElementById('m-religion')?.value || '').trim(), nationality: (document.getElementById('m-nationality')?.value || '').trim(), bform: (document.getElementById('m-bform')?.value || '').trim(), fees: document.getElementById('m-fees')?.value || 'Pending', father: (document.getElementById('m-father')?.value || '').trim(), fphone: (document.getElementById('m-fphone')?.value || '').trim(), cnic: (document.getElementById('m-cnic')?.value || '').trim(), occupation: (document.getElementById('m-occupation')?.value || '').trim(), formNo: (document.getElementById('m-formno')?.value || '').trim(), prevSchool: (document.getElementById('m-prevschool')?.value || '').trim(), mother: (document.getElementById('m-mother')?.value || '').trim(), address: (document.getElementById('m-addr')?.value || '').trim(), whatsapp: wa, photo, familyId, loginEmail, customFee: cfVal === '' ? '' : num(cfVal), vanFee: vfVal === '' ? '' : num(vfVal), arrears: arrVal === '' ? '' : num(arrVal) };
      ...
      if (editId) {
        ...
        if (!editId) {
          ...
          if (newStu) {
            setTimeout(() => {
              openModal(`
              <h3><span class="material-symbols-outlined" style="font-size:inherit;vertical-align:-2px;">celebration</span> Student Added!</h3>
              ...
              <div class="m-btns">
                <button class="btn btn-p" style="flex:2;" onclick="closeModal();printAdmissionForm('${newStu.id}','${classId}');"><span class="material-symbols-outlined" style="font-size:inherit;vertical-align:-2px;">print</span> Print Admission Form</button>
    ...
    function delStu(sid, cid) {
      ...
      DB.settings.terms.forEach(t => { delete (DB.grades[cid + '_' + t.id] || {})[sid]; }); delete (DB.attendance[cid] || {})[sid]; delete DB.fees.payments[sid]; delete DB.fees.monthlyLedger[sid];
  ...
  <div class="score-mini">${getClsSubjects(cid).map(sub => { const v = (DB.grades[cid + '_' + curTerm]?.[sid] || {})[sub.id] || 0; return `<div class="sm-row"><div class="sm-lbl">${sub.icon} ${sub.name}</div><div class="sm-bar"><div class="sm-fill" style="width:${v}%;background:${sub.color};"></div></div><div class="sm-val" style="color:${sub.color};">${v || '-'}</div></div>`; }).join('')}</div>
  <div class="m-btns" style="margin-top:13px;flex-wrap:wrap;">
    ${curRole === 'admin' ? `<button class="btn btn-gh btn-sm" style="color:#d4af37;font-weight:700;" onclick="makeStudentOfMonth('${sid}','${cid}')">⭐ Set Student of Month</button>` : ''}
    ${curRole !== 'teacher' ? `<button class="btn btn-p btn-sm" onclick="closeModal();openAddStudentModal('${sid}')"> Edit</button>` : ''}
    ...
    function findClassFromBulk(value, fallbackId) {
      ...
      const found = DB.classes.find(c => {
        ...
        const cnum = cn.replace(/\D/g, '');
        return cid === v || cn === v || (c.name + ' ' + (c.section || '')).trim().toLowerCase() === v || (numOnly && cnum === numOnly);
      });
      return found?.id || fallbackId;
    }

    function addBulkStudent(row, fallbackClassId, index) {
      const name = row.name || row.student_name || row.full_name || row.studentname || row.fullname;
      if (!name) return false;
      const classId = findClassFromBulk(row.class || row.class_id || row.class_name || row.classname || row.grade || row.standard, fallbackClassId);
      if (!DB.students[classId]) DB.students[classId] = [];
      const roll = row.roll || row.roll_no || row.roll_number || row.rollno || row.rollnumber || row.gr_no || row.grno || row.gr_number || row.id || String(DB.students[classId].length + 1 + index).padStart(3, '0');
      const clsName = (DB.classes.find(c => c.id === classId)?.name || 'class').toLowerCase().replace(/\s/g, '');
      const loginEmail = (row.email || row.student_email || row.studentemail || (String(roll).toLowerCase() + '.' + clsName + '@school.edu')).toLowerCase();
      const familyName = row.parent_name || row.family || row.family_name || row.familyname || '';
      const parentEmail = (row.parent_email || row.parentemail || row.guardian_email || '').toLowerCase();
      let familyId = null;
      if (familyName || parentEmail) {
        ...
        if (!DB.families[familyId]) DB.families[familyId] = { id: familyId, parentName: familyName || parentEmail || (name + ' Family'), phone: row.parent_phone || row.father_phone || row.phone || row.contact || '', email: parentEmail, address: row.address || '', whatsapp: row.whatsapp || '', studentIds: [] };
      }
      function num(v) { return Number(String(v || 0).replace(/[^0-9.-]+/g, '')) || 0; }
      const rawFeeStatus = row.fee_status || row.feestatus || (row.fees && isNaN(Number(String(row.fees).replace(/,/g,''))) ? row.fees : '');
      const fees = (rawFeeStatus && String(rawFeeStatus).toLowerCase() === 'paid') ? 'Paid' : 'Pending';
      const rawCustomFee = row.monthly_fee || row.monthlyfee || row.monthly || row.custom_fee || row.customfee || row.fee_amount || row.fee || row.tuition_fee || row.tuitionfee || row.tuition || row.fees_amount || row.monthly_dues || (row.fees && !isNaN(Number(String(row.fees).replace(/,/g,''))) ? row.fees : '');
      const customFee = parseFeeVal(rawCustomFee);
      const rawVanFee = row.van_fee || row.vanfee || row.transport_fee || row.transportfee || row.bus_fee || row.busfee || row.van;
      const vanFee = parseFeeVal(rawVanFee);
      const rawArrears = row.previous_arrears || row.previous_arrear || row.arrears || row.arrear || row.previous_arrears_balance || row.arrear_balance || row.prev_arrears;
      const arrears = num(rawArrears);

      const studentData = {
        name, roll, classId,
        gender: row.gender || row.sex || '',
        dob: row.dob || row.date_of_birth || row.dateofbirth || row.birth_date || '',
        blood: row.blood || row.blood_group || row.bloodgroup || '',
        religion: (row.religion || '').trim(),
        nationality: (row.nationality || '').trim(),
        cnic: (row.cnic || row.father_cnic || row.fathercnic || '').trim(),
        bform: (row.bform || row.b_form || row.bform_no || '').trim(),
        occupation: (row.occupation || row.father_occupation || '').trim(),
        formNo: (row.form_no || row.formno || row.admission_form_no || '').trim(),
        fees,
        father: row.father_name || row.father || row.fathername || row.parent_name || row.parentname || row.guardian_name || '',
        fphone: row.father_phone || row.fatherphone || row.parent_phone || row.parentphone || row.phone || row.mobile || row.contact || row.contact_no || '',
        mother: row.mother_name || row.mother || row.mothername || '',
        address: row.address || row.home_address || row.residence || row.addr || '',
        whatsapp: row.whatsapp || row.whatsapp_no || row.whatsapp_number || row.wa || '',
        photo: row.photo || row.photo_url || row.image || row.image_url || row.picture || '',
        familyId, loginEmail, customFee, vanFee, arrears,
        previousSchool: row.previous_school || row.previousschool || row.prev_school || row.prevschool || row.last_school || '',
        notes: row.notes || row.message || row.remarks || row.comment || '',
        attendance: 0
      };

      const norm = s => String(s || '').replace(/\s+/g, '').toLowerCase();
      const existingStudent = allStus().find(s => {
        if (s.roll && roll && String(s.roll).trim() === String(roll).trim()) return true;
        if (s.name && norm(s.name) === norm(name) && s.classId === classId) return true;
        return false;
      });
      let newId;
      if (existingStudent) {
        newId = existingStudent.id;
        const targetClassId = existingStudent.classId || classId;
        
        // Preserve existing financial data if the CSV fields were left blank
        if (rawCustomFee === undefined || rawCustomFee === '') studentData.customFee = existingStudent.customFee;
        if (rawVanFee === undefined || rawVanFee === '') studentData.vanFee = existingStudent.vanFee;
        if (rawArrears === undefined || rawArrears === '') studentData.arrears = existingStudent.arrears;

        Object.assign(existingStudent, studentData, { classId: targetClassId });
      } else {
        newId = uid();
      ...
      DB.admissions = DB.admissions || [];
      DB.admissions.push({ id: uid(), studentName: name, classId, className: DB.classes.find(c => c.id === classId)?.name || '', father: studentData.father, mother: studentData.mother, phone: studentData.fphone, whatsapp: studentData.whatsapp, email: loginEmail, dob: studentData.dob, gender: studentData.gender, previousSchool: studentData.previousSchool, address: studentData.address, notes: studentData.notes, status: 'approved', convertedStudentId: newId, createdAt: today() });
      return true;
    }
  ...
  <div class="ibox">Upload <b>students_bulk_template.csv</b>. Use the <b>photo</b> column for an image URL. <a href="students_bulk_template.csv" download="students_bulk_template.csv" class="btn btn-gh btn-xs" style="margin-top:6px;display:inline-flex;align-items:center;gap:4px;text-decoration:none;"> Download CSV Template</a></div>
  ...
  <div class="m-btns">
    ...
    function importBulkRows(rows) {
      ...
      let dedupCount = 0;
      Object.keys(DB.students).forEach(cid => {
         const list = DB.students[cid];
         if (!Array.isArray(list)) return;
         const unique = [];
         const seen = new Set();
         for (let i = list.length - 1; i >= 0; i--) {
            const nm = String(list[i].name || '').replace(/\s+/g, '').toLowerCase();
            if (!seen.has(nm)) { seen.add(nm); unique.unshift(list[i]); }
            else { dedupCount++; }
         }
         DB.students[cid] = unique;
      });
      if (dedupCount > 0) console.log(`Cleaned up ${dedupCount} duplicate students.`);

      let added = 0;
      // Debug: log each row's arrears parsing
      const studentRows = rows.filter(r => !r.role || String(r.role).toLowerCase() === 'student');
      let csvArrearsTotal = 0;
      studentRows.forEach((row, i) => {
        const rawA = row.previous_arrears || row.previous_arrear || row.arrears || row.arrear || row.previous_arrears_balance || row.arrear_balance || row.prev_arrears || '';
        const parsed = num(rawA);
        csvArrearsTotal += parsed;
        if (addBulkStudent(row, fallbackClassId, i)) added++;
      });
      const dbArrearsTotal = allStus().reduce((s, x) => s + num(x.arrears || 0), 0);
      alert(`Import Complete!\n\nStudents updated/added: ${added}\nTotal Arrears Found in CSV: PKR ${csvArrearsTotal.toLocaleString()}\n\nIf the arrears sum is lower than you expect, please check your CSV file to ensure all values are entered correctly.`);
      saveDB();
      closeModal();
      showToast(`✅ ${added} students imported. CSV Arrears: PKR ${csvArrearsTotal.toLocaleString()} | DB Arrears: PKR ${dbArrearsTotal.toLocaleString()}`, 'ts');
      goPage('students');
    }
...
<div class="ph ph-row fu">
  ...
  ${clsBar('switchAttCls')}
  ${sts.length === 0 ? `<div class="card"><div class="empty"><div class="empty-ic"></div><div class="empty-t">No students in this class</div>${curRole !== 'teacher' ? `<button class="btn btn-p btn-sm" style="margin-top:10px;" onclick="openAddStudentModal()">Add Student</button>` : ''}</div></div>` : `
...
<div style="display:flex;gap:9px;margin-top:9px;align-items:center;font-size:.72rem;flex-wrap:wrap;">
  ...
  <div class="m-btns"><button class="btn btn-p" onclick="saveLateThresh()"> Save</button><button class="btn btn-gh" onclick="closeModal()">Cancel</button></div>`);
    ...
    /* ------------------------------- GRADES ------------------------------- */
    function pgGrades() {
      if (curCls === 'staff' || !(DB.classes || []).find(c => c.id === curCls)) { curCls = DB.classes[0]?.id || 'c1'; }
      const sts = clsStus(curCls); const cls = DB.classes.find(c => c.id === curCls);
      return `
<div class="ph ph-row fu">
...
<div class="card fu d2" style="padding:0;overflow:hidden;"><div class="tw"><table>
  <thead><tr><th>Roll</th><th>Student</th>${getClsSubjects(curCls).map(s => `<th style="text-align:center;">${s.icon}<br/><span style="font-weight:400;">${s.code}</span></th>`).join('')}<th>Avg</th><th>Grade</th></tr></thead>
  <tbody>${sts.map(s => {
        ...
        const filled = getClsSubjects(curCls).map(sub => g[sub.id] || 0).filter(v => v > 0);
    ...
    <td style="font-weight:600;font-size:.8rem;white-space:nowrap;">${s.name}</td>
      ${getClsSubjects(curCls).map(sub => `<td style="text-align:center;padding:5px 4px;"><input class="si" type="number" min="0" max="100" value="${g[sub.id] || ''}" placeholder="-" style="width:50px;text-align:center;background:var(--card2);border:1.5px solid var(--border);border-radius:7px;padding:6px 4px;font-size:.84rem;font-weight:700;color:var(--ink);outline:none;font-family:var(--font);" oninput="setGrade('${s.id}','${sub.id}',this.value,this)" onfocus="this.style.borderColor='var(--p)';this.style.background='#fff'" onblur="this.style.borderColor='';this.style.background='';"/></td>`).join('')}
    ...
    function getTrueInitialArrears(sid) {
        const s = allStus().find(x => x.id === sid);
        if (!s) return 0;
        let earliestMonth = null;
        let earliestSnapshot = null;
        if (DB.fees.monthlyLedger && DB.fees.monthlyLedger[sid]) {
            for (let m in DB.fees.monthlyLedger[sid]) {
                let row = DB.fees.monthlyLedger[sid][m];
                if (row.paid && row.arrearsSnapshot !== undefined) {
                    if (!earliestMonth || m < earliestMonth) {
                        earliestMonth = m;
                        earliestSnapshot = num(row.arrearsSnapshot);
                    }
                }
            }
        }
        if (earliestMonth) return earliestSnapshot;
        return num(s.arrears || 0);
    }

    function getLedgerArrears(sid, upToMonth) {
        const s = allStus().find(x => x.id === sid);
        if (!s) return 0;
        let balance = getTrueInitialArrears(sid);
        let m = '2026-08';
        while (m < upToMonth) {
            let tuition = monthlyFeeAmount(s);
            let van = num(s.vanFee || 0);
            let itemsTotal = 0;
            const mNames = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
            const mIndex = parseInt(m.split('-')[1], 10) - 1;
            const monthName = mNames[mIndex];
            DB.fees.items.forEach(f => {
                if (f.id === 'f1') return;
                if (f.applyMonth && f.applyMonth !== 'All' && f.applyMonth !== monthName) return;
                itemsTotal += num(f.amount);
            });
            balance += (tuition + van + itemsTotal);
            if (DB.fees.monthlyLedger && DB.fees.monthlyLedger[sid] && DB.fees.monthlyLedger[sid][m] && DB.fees.monthlyLedger[sid][m].paid) {
                balance -= num(DB.fees.monthlyLedger[sid][m].amountPaid || 0);
            }
            m = getNextMonth(m);
        }
        return balance;
    ...
    function studentFeeSummary(sid) {
      ...
      const mandatory = allItems.filter(f => f.mandatory);
      const cm = monthKey();
      const isBeforeAugust = cm < '2026-08';

      let paidAmount = 0;
      if (DB.fees.monthlyLedger && DB.fees.monthlyLedger[sid]) {
        Object.values(DB.fees.monthlyLedger[sid]).forEach(m => {
           if (m.paid) paidAmount += num(m.amountPaid);
        });
      }

      const arrears = isBeforeAugust ? num(s?.arrears || 0) : getLedgerArrears(sid, cm);
      let totalDue = isBeforeAugust ? arrears : getLedgerArrears(sid, getNextMonth(cm));
      ...
      if (!isBeforeAugust) {
          ...
          let itemsTotal = 0;
          const mNames = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
          const mIndex = parseInt(cm.split('-')[1], 10) - 1;
          const monthName = mNames[mIndex];
          DB.fees.items.forEach(f => {
              if (f.id === 'f1') return;
              if (f.applyMonth && f.applyMonth !== 'All' && f.applyMonth !== monthName) return;
          ...
          currentMonthBill = tuition + van + itemsTotal;
      }

      let truePendingTotal = Math.max(0, totalDue);
      let pendingArrears = Math.max(0, truePendingTotal - currentMonthBill);
      let pendingMonthly = truePendingTotal - pendingArrears;

      return { payments: DB.fees.payments[sid] || {}, mandatory, mandatoryPaid: 0, paidAmount, dueAmount: totalDue, arrears, pendingArrears, pendingMonthly, totalAmount: paidAmount + totalDue };
    }

    function filterFeeTable() {
        const classQ = document.getElementById('fee-class-filter')?.value.toLowerCase() || '';
        const textQ = document.getElementById('fee-search-filter')?.value.toLowerCase().trim() || '';
        const tbody = document.getElementById('fees-tbody');
        if (!tbody) return;
        tbody.querySelectorAll('tr').forEach(tr => {
            const rowText = tr.textContent.toLowerCase();
            const classCellText = tr.children[1] ? tr.children[1].textContent.toLowerCase().trim() : '';
            const matchesClass = !classQ || classCellText === classQ;
            const matchesText = !textQ || rowText.includes(textQ);
            tr.style.display = (matchesClass && matchesText) ? '' : 'none';
        });
    }
    ...
    function pgFees() {
      ...
      const pending = summaries.reduce((sum, x) => sum + Math.max(0, x.dueAmount), 0);
      const pendingArrears = summaries.reduce((sum, x) => sum + (x.pendingArrears || 0), 0);
      const pendingMonthly = summaries.reduce((sum, x) => sum + (x.pendingMonthly || 0), 0);
      const totalArrears = all.reduce((sum, s) => sum + num(s.arrears || 0), 0);
...
<div class="fee-hero fu">
  <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;">
    <div>
      <div style="font-size:.7rem;opacity:.65;">Total Collection</div>
      <div style="font-family:var(--head);font-size:1.7rem;font-weight:700;margin:4px 0;">${fmtMoney(collected)}</div>
      <div style="font-size:.7rem;opacity:.6;">${fullyPaid} fully paid - ${all.length - fullyPaid} pending</div>
    </div>
    <div style="text-align:right;">
      <div style="font-size:.7rem;opacity:.65;">${isBeforeAugust ? 'Total Prev. Arrears' : 'Total Pending Balance'}</div>
      <div style="font-family:var(--head);font-size:1.1rem;font-weight:700;margin-top:3px;color:var(--o);">${fmtMoney(isBeforeAugust ? totalArrears : pending)}</div>
      ${isBeforeAugust ? 
          `<div style="font-size:.68rem;opacity:.62;">Monthly fee starts Aug 2026</div>` : 
          `<div style="font-size:.72rem;opacity:.8;display:flex;gap:10px;justify-content:flex-end;margin-top:2px;">
             <span>Monthly <b style="color:var(--o);">${fmtMoney(pendingMonthly)}</b></span>
             <span>Arrears <b style="color:var(--r);">${fmtMoney(pendingArrears)}</b></span>
           </div>`
      }
    </div>
  </div>
  <div class="fh-bar"><div class="fh-fill" style="width:${collected + pending ? Math.round(collected / (collected + pending) * 100) : 0}%;"></div></div>
  <div style="display:flex;justify-content:space-between;font-size:.68rem;opacity:.6;margin-top:4px;"><span>${collected + (isBeforeAugust ? totalArrears : pending) ? Math.round(collected / (collected + (isBeforeAugust ? totalArrears : pending)) * 100) : 0}% collected</span><span>${isBeforeAugust ? fmtMoney(totalArrears) + ' total arrears' : fmtMoney(pending) + ' pending'}</span></div>
</div>
<div class="g2">
  <div class="card fu d2">
  ...
  <div class="card fu d3" style="padding:0;overflow:hidden;">
    <div class="ch" style="padding:13px 14px 9px;">
      <div class="ct"> Student Fee Status</div>
      <div style="margin-left:auto; display:flex; gap:10px; align-items:center;">
        <select id="fee-class-filter" class="fi" onchange="filterFeeTable()" style="padding:6px 10px; width:160px;">
          <option value="">All Classes</option>
          ${DB.classes.map(c => `<option value="${escHtml(c.name)}">${escHtml(c.name)}</option>`).join('')}
        </select>
        <input type="text" id="fee-search-filter" class="fi" placeholder="Search student or roll..." onkeyup="filterFeeTable()" style="width:200px; padding:6px 10px;"/>
      </div>
    </div>
    <div class="tw" style="border:none;"><table>
      ...
      <tbody id="fees-tbody">${all.map(s => {
        const sum = studentFeeSummary(s.id);
        const complete = sum.mandatory.length && sum.mandatoryPaid === sum.mandatory.length;
        const cls = DB.classes.find(c => c.id === s.classId);
        const arrBalance = num(s.arrears || 0);
        return `<tr><td><div style="font-weight:700;font-size:.78rem;">${s.name}</div><div style="font-size:.63rem;color:var(--ink4);">Roll ${s.roll}</div></td><td style="font-size:.72rem;">${cls?.name || '-'}</td><td><span class="badge ${complete ? 'bg2' : 'br2'}">${sum.mandatoryPaid}/${sum.mandatory.length} paid</span></td><td><div style="font-weight:800;color:${arrBalance > 0 ? 'var(--r)' : 'var(--g)'};font-size:.85rem;">${fmtMoney(arrBalance)}</div><div style="font-size:.65rem;color:var(--ink4);margin-top:1px;">Monthly (Aug+): ${fmtMoney(monthlyFeeAmount(s))}</div></td>
        <td><div style="display:flex;gap:3px;flex-wrap:wrap;">
    ...
    function calcStudentFeeBill(sid, month) {
      const s = allStus().find(x => x.id === sid);
      if (!s) return null;
      
      const mNames = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
      const dateParts = month ? month.split('-') : [];
      const monthIndex = dateParts.length === 2 ? parseInt(dateParts[1], 10) - 1 : new Date().getMonth();
      ...
      const isPaid = mRow ? mRow.paid : false;
      const amountPaid = mRow ? num(mRow.amountPaid) : 0;
      
      const tuition = monthlyFeeAmount(s);
      const van = num(s.vanFee || 0);
      const arrears = getLedgerArrears(sid, month);
      
      let itemsTotal = 0;
      let breakdownHtml = `<div style="display:flex;justify-content:space-between;margin-bottom:4px;"><span>Monthly Fee:</span> <b>${fmtMoney(tuition)}</b></div>`;
      if (van > 0) breakdownHtml += `<div style="display:flex;justify-content:space-between;margin-bottom:4px;"><span>Van Transport Fee:</span> <b>${fmtMoney(van)}</b></div>`;
      if (arrears !== 0) breakdownHtml += `<div style="display:flex;justify-content:space-between;margin-bottom:4px;color:${arrears > 0 ? 'var(--o)' : 'var(--g)'};"><span>${arrears > 0 ? 'Previous Arrears' : 'Advance Credit'}:</span> <b>${fmtMoney(Math.abs(arrears))}</b></div>`;

      let applicableItems = [];
      DB.fees.items.forEach(f => {
        if (f.id === 'f1') return; // Handled as tuition above
        if (f.applyMonth && f.applyMonth !== 'All' && f.applyMonth !== monthName) return;
        applicableItems.push(f);
        itemsTotal += num(f.amount);
        breakdownHtml += `<div style="display:flex;justify-content:space-between;margin-bottom:4px;"><span>${escHtml(f.name)}:</span> <b>${fmtMoney(f.amount)}</b></div>`;
      });

      const totalDue = Math.max(0, tuition + van + arrears + itemsTotal);
      
      return { tuition, van, arrears, itemsTotal, applicableItems, totalDue, isPaid, amountPaid, mRow, breakdownHtml };
    }
    ...
    function openStudentFeeModal(sid, targetMonth = null) {
      ...
      const bill = calcStudentFeeBill(sid, month);
      ...
      openModal(`
        ...
        <div class="ibox" style="margin-bottom:12px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;">
          <div>${escHtml(cls?.name || '')} - Roll ${escHtml(s.roll || '-')} - Arrears <b style="color:${num(s.arrears) ? 'var(--r)' : 'var(--g)'};">${fmtMoney(num(s.arrears))}</b></div>
        ...
        <div style="background:var(--card2);padding:14px;border-radius:12px;margin-bottom:15px;font-size:0.85rem;display:flex;flex-direction:column;gap:8px;">
           <div style="font-weight:700;color:var(--ink3);margin-bottom:4px;">Ledger Breakdown: ${month}</div>
           ${bill.breakdownHtml}
           <hr style="border:0;border-top:1px dashed var(--border);margin:4px 0;" />
           <div style="display:flex;justify-content:space-between;color:var(--p);font-size:1.1rem;font-weight:900;"><span>Total Due:</span> <span>${fmtMoney(bill.totalDue)}</span></div>
        </div>

        ${bill.isPaid ? `
           <div style="text-align:center;padding:15px;background:var(--pg2);color:var(--g);border-radius:10px;font-weight:800;margin-bottom:15px;">
              ${fmtMoney(bill.amountPaid)} Paid on ${fmtDate(bill.mRow.paidOn)}
           </div>
           <div class="m-btns" style="flex-wrap:wrap;">
              <button class="btn btn-gh btn-sm" onclick="unmarkStudentPayment('${sid}', '${month}')">Undo Payment</button>
              <button class="btn btn-g btn-sm" onclick="printFeeReceipt('${sid}', '${month}')">Print Receipt</button>
              <button class="btn btn-gh btn-sm" onclick="closeModal()">Close</button>
           </div>
        ` : `
           <div class="fg">
              <label class="fl">Amount Received Today (Partial Payments roll over to Arrears)</label>
              <input class="fi" type="number" id="ms-receive-amt" value="${bill.totalDue}" />
           </div>
           <div class="m-btns" style="flex-wrap:wrap;">
              <button class="btn btn-p btn-sm" onclick="receiveStudentPayment('${sid}', '${month}', ${bill.totalDue}, ${bill.arrears})">Receive Payment</button>
              <button class="btn btn-gh btn-sm" onclick="closeModal()">Cancel</button>
           </div>
        `}
      `);
    }

    function receiveStudentPayment(sid, month, totalDue, snapshotArrears) {
      const s = allStus().find(x => x.id === sid); if (!s) return;
      const amountPaid = num(document.getElementById('ms-receive-amt')?.value || 0);
      
      if (!DB.fees.monthlyLedger) DB.fees.monthlyLedger = {};
      if (!DB.fees.monthlyLedger[sid]) DB.fees.monthlyLedger[sid] = {};
      
      let saveSnapshot = snapshotArrears;
      if (Object.keys(DB.fees.monthlyLedger[sid]).filter(m => DB.fees.monthlyLedger[sid][m].paid).length === 0) {
          saveSnapshot = num(s.arrears || 0);
      } else {
          saveSnapshot = num(s.arrears || 0);
      }
      
      DB.fees.monthlyLedger[sid][month] = { 
         month, 
         paid: true, 
         paidOn: today(), 
         totalDue: totalDue, 
         amountPaid: amountPaid, 
         arrearsSnapshot: saveSnapshot
      };
      
      s.fees = getLedgerArrears(sid, getNextMonth(month)) <= 0 ? 'Paid' : 'Pending';
      
      saveDB();
      showToast('Payment received & ledger updated');
      goPage('fees');
      printFeeReceipt(sid, month);
    }
    
    function unmarkStudentPayment(sid, month) {
       const s = allStus().find(x => x.id === sid); if (!s) return;
       if(!confirm('Undo payment? The ledger will automatically recalculate balances.')) return;
       const mRow = DB.fees.monthlyLedger[sid][month];
       if(mRow) {
          delete DB.fees.monthlyLedger[sid][month];
          saveDB();
          showToast('Payment undone');
          openStudentFeeModal(sid, month);
          goPage('fees');
       }
    }
    ...
    function printFeeReceipt(sid, month) {
      ...
      <style>
        body { font-family: sans-serif; padding: 20px; color: #111; max-width: 600px; margin: 0 auto; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
        ...
        .totals { text-align: right; font-size: 16px; font-weight: bold; margin-bottom: 5px; }
      </style></head><body>
      <div class="header">
        ...
        <h2 style="margin:10px 0 0 0;">Fee Receipt (${isPaid ? 'PAID' : 'PENDING'})</h2>
      </div>
      <div class="details">
        <div>
        ...
        <div style="text-align:right;">
          <b>Date:</b> ${fmtDate(isPaid ? bill.mRow.paidOn : today())}<br>
          <b>Billing Month:</b> ${month}
        </div>
      </div>
      <table class="table">
        <tr><th>Fee Description</th><th>Amount</th></tr>
        <tr><td>Monthly Tuition</td><td>${fmtMoney(bill.tuition)}</td></tr>
        ${bill.van > 0 ? `<tr><td>Van Transport Fee</td><td>${fmtMoney(bill.van)}</td></tr>` : ''}
        ${bill.arrears > 0 ? `<tr><td style="color:#d32f2f;">Previous Arrears</td><td style="color:#d32f2f;">${fmtMoney(bill.arrears)}</td></tr>` : ''}
        ${bill.applicableItems.map(f => `<tr><td>${escHtml(f.name)}</td><td>${fmtMoney(f.amount)}</td></tr>`).join('')}
      </table>
      <div class="totals">Total Billed: ${fmtMoney(bill.totalDue)}</div>
      ${isPaid ? `<div class="totals" style="color:green;font-size:18px;">Amount Paid: ${fmtMoney(bill.amountPaid)}</div>
                   <div class="totals" style="color:red;font-size:16px;">Remaining Arrears: ${fmtMoney(bill.mRow.arrearsRolledOver)}</div>` : 
                   `<div class="totals" style="color:red;font-size:18px;">Balance Due: ${fmtMoney(bill.totalDue)}</div>`}
      ...
      <div style="text-align: center; font-size: 12px; color: #777; margin-top: 20px; border-top: 1px solid #eee; padding-top: 10px;">Thank you for your payment!</div>
      <scr` + `ipt>
  ...
  <div class="m-btns" style="flex-wrap:wrap;">
    ...
    function openFeeStructModal() {
      openModal(`
  <h3>? Fee Structure</h3>
  <div id="fee-list">${DB.fees.items.map((f, i) => `<div style="background:var(--card2);border:1px solid var(--border);border-radius:9px;padding:11px;margin-bottom:8px;"><div class="fg2"><div class="fg"><label class="fl">Fee Name</label><input class="fi" value="${f.name}" onchange="DB.fees.items[${i}].name=this.value" style="font-size:.82rem;"/></div><div class="fg"><label class="fl">Amount (${DB.settings?.currency || 'PKR'})</label><input class="fi" type="number" value="${f.amount}" onchange="DB.fees.items[${i}].amount=Number(this.value)" style="font-size:.82rem;"/></div></div><div class="fg2"><div class="fg"><label class="fl">Due Date</label><input class="fi" value="${f.due}" onchange="DB.fees.items[${i}].due=this.value" style="font-size:.82rem;"/></div><div class="fg"><label class="fl">Apply Month</label><select class="fi" onchange="DB.fees.items[${i}].applyMonth=this.value" style="font-size:.82rem;"><option value="All" ${f.applyMonth !== 'March' && f.applyMonth !== 'December' ? 'selected' : ''}>Every Month</option><option value="March" ${f.applyMonth === 'March' ? 'selected' : ''}>March</option><option value="December" ${f.applyMonth === 'December' ? 'selected' : ''}>December</option></select></div></div><div class="fg2"><div class="fg" style="display:flex;align-items:flex-end;gap:8px;"><label style="display:flex;align-items:center;gap:5px;font-size:.76rem;font-weight:600;cursor:pointer;padding-bottom:2px;"><input type="checkbox" ${f.mandatory ? 'checked' : ''} onchange="DB.fees.items[${i}].mandatory=this.checked"/> Mandatory</label>${f.id === 'f1' ? '<div style="font-size:.65rem;color:var(--ink4);margin-bottom:5px;">System Item</div>' : `<button class="btn btn-xs" style="background:var(--rl);color:var(--r);margin-bottom:0;" onclick="delFeeItem(${i})">Remove</button>`}</div></div></div>`).join('')}</div>
  ...
  <div class="m-btns" style="margin-top:10px;"><button class="btn btn-p" onclick="saveFeeStruct()"> Save</button><button class="btn btn-gh" onclick="closeModal()">Close</button></div>`);
    ...
    function addFeeItem() { DB.fees.items.push({ id: uid(), name: 'New Fee', amount: 1000, due: '5th', mandatory: false, type: 'monthly', applyMonth: 'All' }); openFeeStructModal(); }
...
<div class="g3 fu d1">${DB.classes.map(c => { const sts = clsStus(c.id); const avg = sts.length ? Math.round(sts.reduce((a, s) => a + avgStu(c.id, s.id), 0) / sts.length) : 0; return `<div class="card" style="cursor:pointer;padding:13px;transition:all .17s;" onclick="switchCls('${c.id}');goPage('students')" onmouseover="this.style.transform='translateY(-2px)';this.style.borderColor='var(--p2)'" onmouseout="this.style.transform='';this.style.borderColor=''"><div style="display:flex;justify-content:space-between;margin-bottom:8px;"><div style="width:28px;height:28px;border-radius:7px;background:${gc(c.name)};display:flex;align-items:center;justify-content:center;font-size:.7rem;font-weight:800;color:#fff;">${c.name.charAt(0)}</div><div style="display:flex;gap:4px;"><button class="btn btn-xs btn-gh" onclick="event.stopPropagation();openPromoteModal('${c.id}')">Promote</button> <button class="btn btn-xs btn-gh" onclick="event.stopPropagation();openAddClsModal('${c.id}')">Edit</button> <button class="btn btn-xs btn-wa" onclick="event.stopPropagation();broadcastClass('${c.id}')">Broadcast</button></div></div><div style="font-weight:700;font-size:.85rem;">${escHtml(c.name)}${c.section ? ' ' + escHtml(c.section) : ''}</div><div style="font-size:.68rem;color:var(--ink3);margin:2px 0 8px;">${sts.length} students - ${c.level === 'primary' ? 'Primary' : 'Secondary'}</div><div class="pb" style="height:4px;"><div class="pf" style="width:${avg}%;background:${sCol(avg)};height:4px;"></div></div><div style="display:flex;justify-content:space-between;margin-top:4px;font-size:.66rem;font-weight:700;"><span style="color:${sCol(avg)};">${avg}% avg</span><span style="color:var(--p);">View</span></div></div>`; }).join('')}<div class="card" style="display:flex;align-items:center;justify-content:center;border:2px dashed var(--border2);background:transparent;cursor:pointer;min-height:120px;" onclick="openAddClsModal()"><div style="text-align:center;color:var(--ink4);"><div style="font-size:1.3rem;">+</div><div style="font-size:.74rem;font-weight:600;margin-top:3px;">Add Class</div></div></div></div>`;
  ...
  <div class="m-btns"><button class="btn btn-p" onclick="saveCls('${eid || ''}')"> ${c ? 'Update' : 'Add'}</button>${c ? `<button class="btn btn-xs" style="background:var(--rl);color:var(--r);" onclick="delCls('${c.id}')">Delete</button>` : ''}<button class="btn btn-gh" onclick="closeModal()">Cancel</button></div>`);
    ...
    function delCls(id) { if (!confirm('Delete class and all students?')) return; (DB.students[id] || []).forEach(s => { delete DB.fees.payments[s.id]; delete DB.fees.monthlyLedger[s.id]; }); DB.classes = DB.classes.filter(c => c.id !== id); delete DB.students[id]; DB.settings.terms.forEach(t => delete DB.grades[id + '_' + t.id]); delete DB.attendance[id]; saveDB(); closeModal(); showToast('Deleted'); goPage('classes'); }
...
<div class="ph ph-row fu"><div><div class="ph-tag"> Notices</div><h1>Notice Board</h1></div>${curRole === 'admin' || curRole === 'teacher' ? `<button class="btn btn-p" onclick="openAddNoticeModal()">+ Post Notice</button>` : ''}</div>
  ...
  <div class="m-btns"><button class="btn btn-p" onclick="saveNotice('${eid || ''}')"> ${n ? 'Update' : 'Post'}</button><button class="btn btn-gh" onclick="closeModal()">Cancel</button></div>`);
    ...
    function ensurePeriods() {
      if (!DB.periods || DB.periods.length < 9) {
        DB.periods = [
    ...
    function pgTimetable() {
      ...
      const tt = DB.timetable[curCls]; 
      DAYS.forEach(d => { if (!tt[d]) tt[d] = Array(DB.periods.length).fill(''); });
      
      const teachers = DB.users.filter(u => u.role === 'teacher');
      
      return `
<div class="ph ph-row fu"><div><div class="ph-tag"> Timetable</div><h1>Weekly Schedule</h1><div class="ph-sub">${cls?.name || ''} - Edit times in headers, assign subjects and teachers</div></div><button class="btn btn-g btn-sm" onclick="saveTT()"> Save</button></div>
  ${clsBar('switchTTCls')}
<div class="card fu d2" style="padding:0;overflow:hidden;"><div class="tw"><table style="min-width:680px;">
  <thead><tr><th style="min-width:140px;">Period</th>${DAYS.map(d => `<th style="min-width:140px;text-align:center;">${d}</th>`).join('')}</tr></thead>
  <tbody>${DB.periods.map((p, pi) => `<tr>
    <td style="background:var(--card2);white-space:nowrap;padding:8px;">
      <div style="font-weight:700;font-size:.78rem;color:var(--ink3);">${p.name}</div>
      <input type="text" class="fi" value="${p.time}" onchange="updatePeriodTime(${pi}, this.value)" placeholder="HH:MM - HH:MM" style="font-size:.65rem;padding:3px;margin-top:4px;width:100px;"/>
    </td>
    ${p.name === 'Break' ? `<td colspan="${DAYS.length}" style="background:var(--card3);text-align:center;font-size:.75rem;font-weight:700;color:var(--ink4);letter-spacing:1px;">☕ Break</td>` : DAYS.map(d => { 
        let cell = tt[d]?.[pi];
        if (typeof cell === 'string') cell = { subject: cell, teacherId: '' };
        if (!cell) cell = { subject: '', teacherId: '' };
        return `<td style="padding:6px;vertical-align:top;background:var(--card);">
          <select class="fi" style="font-size:.7rem;padding:5px 8px;width:100%;margin-bottom:4px;font-weight:600;" onchange="setTT('${d}',${pi},'subject',this.value)">
            <option value="">- Subject -</option>
            ${DB.subjects.map(s => `<option value="${s.name}" ${cell.subject === s.name ? 'selected' : ''}>${s.icon} ${s.name}</option>`).join('')}
          </select>
          <select class="fi" style="font-size:.68rem;padding:4px 8px;width:100%;" onchange="setTT('${d}',${pi},'teacher',this.value)">
            <option value="">- Teacher -</option>
            ${teachers.map(t => `<option value="${t.id}" ${cell.teacherId === t.id ? 'selected' : ''}>${t.name}</option>`).join('')}
          </select>
        </td>`; 
    }).join('')}
  </tr>`).join('')}
    ...
    function updatePeriodTime(pi, val) { DB.periods[pi].time = val; saveDB(); }
    
    function setTT(day, pi, field, val) { 
        if (!DB.timetable[curCls]) DB.timetable[curCls] = {}; 
        if (!DB.timetable[curCls][day]) DB.timetable[curCls][day] = Array(DB.periods.length).fill(''); 
        let cell = DB.timetable[curCls][day][pi];
        if (typeof cell === 'string') cell = { subject: cell, teacherId: '' };
        if (!cell) cell = { subject: '', teacherId: '' };
        cell[field] = val;
        DB.timetable[curCls][day][pi] = cell; 
    }
    ...
    function pgMyTimetable() {
      ensurePeriods();
      return `
<div class="ph ph-row fu"><div><div class="ph-tag"> Timetable</div><h1>My Weekly Schedule</h1><div class="ph-sub">Your classes and subjects</div></div></div>
<div class="card fu d2" style="padding:0;overflow:hidden;"><div class="tw"><table style="min-width:600px;">
  <thead><tr><th style="min-width:140px;">Period</th>${DAYS.map(d => `<th style="min-width:120px;text-align:center;">${d}</th>`).join('')}</tr></thead>
  <tbody>${DB.periods.map((p, pi) => `<tr>
    <td style="background:var(--card2);white-space:nowrap;padding:8px;">
      <div style="font-weight:700;font-size:.78rem;color:var(--ink3);">${p.name}</div>
      <div style="font-size:.65rem;color:var(--ink4);margin-top:2px;">${p.time}</div>
    </td>
    ${p.name === 'Break' ? `<td colspan="${DAYS.length}" style="background:var(--card3);text-align:center;font-size:.75rem;font-weight:700;color:var(--ink4);letter-spacing:1px;">☕ Break</td>` : DAYS.map(d => { 
        // Find if this teacher is assigned to any class at this day and period
        let assignments = [];
        DB.classes.forEach(c => {
          let cell = DB.timetable[c.id]?.[d]?.[pi];
          if (typeof cell === 'string') cell = { subject: cell, teacherId: '' };
          if (cell && cell.teacherId === curUser.id) {
            assignments.push({ class: c.name + (c.section ? ' '+c.section : ''), subject: cell.subject });
          }
        });
        if (assignments.length === 0) return `<td style="padding:6px;text-align:center;color:var(--ink4);font-size:.7rem;background:var(--card);">Free</td>`;
        return `<td style="padding:6px;vertical-align:top;background:var(--card);">${assignments.map(a => `<div style="background:var(--card2);padding:6px;border-radius:6px;margin-bottom:4px;border-left:3px solid var(--p);"><div style="font-weight:700;font-size:.72rem;">${a.class}</div><div style="font-size:.68rem;color:var(--ink3);">${a.subject}</div></div>`).join('')}</td>`;
    }).join('')}
  </tr>`).join('')}
  </tbody></table></div></div>`;
    }
...
<div class="g3">${DB.subjects.map(s => `<div class="card fu" style="padding:12px;display:flex;align-items:center;gap:10px;"><div style="width:36px;height:36px;border-radius:8px;background:${s.color}18;display:flex;align-items:center;justify-content:center;font-size:.65rem;font-weight:800;border:1px solid ${s.color}30;flex-shrink:0;overflow:hidden;letter-spacing:-0.2px;text-align:center;">${cleanIcon(s.icon, s.code || 'SUB')}</div><div style="flex:1;min-width:0;"><div style="font-weight:700;font-size:.83rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${s.name}</div><div style="font-size:.66rem;color:var(--ink4);">${s.code}</div></div><div style="display:flex;flex-direction:column;gap:3px;"><button class="btn btn-xs btn-gh" onclick="openSubjModal('${s.id}')">Edit</button><button class="btn btn-xs" style="background:var(--rl);color:var(--r);" onclick="DB.subjects=DB.subjects.filter(x=>x.id!=='${s.id}');saveDB();showToast('Deleted');goPage('subjects')">Delete</button></div></div>`).join('')}<div class="card fu" style="display:flex;align-items:center;justify-content:center;border:2px dashed var(--border2);background:transparent;cursor:pointer;min-height:72px;" onclick="openSubjModal()"><div style="text-align:center;color:var(--ink4);"><div style="font-size:1.1rem;">+</div><div style="font-size:.7rem;font-weight:600;">Add Subject</div></div></div></div>`;
    ...
    function openSubjModal(eid) { 
      const s = eid ? DB.subjects.find(x => x.id === eid) : null; 
      const classCheckboxes = DB.classes.map(c => {
        const isChecked = (!s?.classIds || s.classIds.length === 0 || s.classIds.includes(c.id)) ? 'checked' : '';
        return `<label style="display:flex;align-items:center;gap:5px;font-size:.75rem;cursor:pointer;"><input type="checkbox" class="subj-cls-cb" value="${c.id}" ${isChecked}/> ${c.name}</label>`;
      }).join('');
      openModal(`<h3>${s ? 'Edit Subject' : 'Add Subject'}</h3><div class="fg2"><div class="fg"><label class="fl">Name <span class="req">*</span></label><input class="fi" id="ms-name" value="${s?.name || ''}" placeholder="Mathematics"/></div><div class="fg"><label class="fl">Code</label><input class="fi" id="ms-code" value="${s?.code || ''}" placeholder="MATH"/></div></div><div class="fg2"><div class="fg"><label class="fl">Icon Text</label><input class="fi" id="ms-icon" value="${cleanIcon(s?.icon, '')}" placeholder="MATH"/></div><div class="fg"><label class="fl">Color</label><input class="fi" type="color" id="ms-color" value="${s?.color || '#2563eb'}" style="height:44px;cursor:pointer;padding:6px;"/></div></div><div class="fg"><label class="fl">Assigned Classes</label><div style="display:flex;flex-wrap:wrap;gap:10px;padding:10px;border:1px solid var(--border);border-radius:8px;background:var(--card2);max-height:150px;overflow-y:auto;">${classCheckboxes}</div></div><div class="m-btns"><button class="btn btn-p" onclick="saveSubj('${eid || ''}')"> ${s ? 'Update' : 'Add'}</button><button class="btn btn-gh" onclick="closeModal()">Cancel</button></div>`); 
    }
    function saveSubj(eid) { 
  const name = (document.getElementById('ms-name')?.value || '').trim(); 
  if (!name) { showToast('Name required', 'te'); return; } 
  const code = (document.getElementById('ms-code')?.value || '').toUpperCase(); 
  const classIds = Array.from(document.querySelectorAll('.subj-cls-cb:checked')).map(cb => cb.value);
  const obj = { name, code, icon: cleanIcon(document.getElementById('ms-icon')?.value || '', code || 'SUB'), color: document.getElementById('ms-color')?.value || '#2563eb', classIds }; 
  if (eid) { 
    const s = DB.subjects.find(x => x.id === eid); 
    if (s) Object.assign(s, obj); 
  } else {
    DB.subjects.push({ id: uid(), ...obj }); 
  }
  saveDB(); closeModal(); showToast('Saved', 'ts'); goPage('subjects'); 
}
...
<div class="card fu d1"><div class="ch"><div class="ct">Select Student</div></div><div style="display:flex;flex-wrap:wrap;gap:7px;">${sts.map(s => `<button class="btn btn-gh btn-sm" onclick="openReportCard('${s.id}','${curCls}')" style="display:flex;align-items:center;gap:7px;"><div class="ava" style="width:22px;height:22px;border-radius:5px;background:${gc(s.name)};font-size:.46rem;flex-shrink:0;">${s.photo ? `<img src="${s.photo}" style="width:100%;height:100%;object-fit:cover;border-radius:5px;"/>` : ini(s.name)}</div>${s.name}</button>`).join('')}</div></div>`}`;
    ...
    function openReportCard(sid, cid) {
      ...
      const rows = getClsSubjects(curCls).map(sub => { const v = grades[sub.id] || 0; return { sub, v }; });
...
<div class="card fu d6">
  ...
  <div class="ibox" style="margin-top:10px;"> Data is saved to Supabase cloud + local browser cache.  </div>
    ...
    function importData(inp) { const f = inp.files[0]; if (!f) return; const r = new FileReader(); r.onload = e => { try { DB = JSON.parse(e.target.result); saveDB(); applyBranding(); buildShell(); updateSOM(); goPage('dashboard'); showToast('? Data imported', 'ts'); } catch { showToast('? Invalid file', 'te'); } }; r.readAsText(f); }
...
<div class="card fu d2"><div class="ch"><div class="ct">Attendance Log</div></div>
    ...
    function classIdCardHtml(stu) {
      ...
      return `<div class="vid-card-wrap" style="display:inline-block; margin:10px;">
        <div class="vid-card">
          <div class="vid-card-inner">
            <div class="ox-header-bg"></div>
            <div class="ox-logo-box"><img src="logo.png" onerror="this.src='https://cdn-icons-png.flaticon.com/512/4774/4774574.png'"></div>
            <div class="ox-logo-text">OXFORD EXCELLENCE ACADEMY</div>
            
            <div class="ox-photo-wrap">${stu.photo ? `<img src="${stu.photo}"/>` : `<span>${ini(stu.name||'?')}</span>`}</div>
            
            <div class="ox-name-wrap">
              <div class="ox-name">${(stu.name||'Student').replace(/^(\S+)\s+(.+)$/, '$1<br>$2')}</div>
              <div class="ox-role">STUDENT</div>
            </div>
            
            <div class="ox-grid">
              <div class="ox-data-item">
                <div class="ox-lbl">ID NO.</div>
                <div class="ox-val">${String(idNo || '001').padStart(3,'0')}</div>
              </div>
              <div class="ox-data-item">
                <div class="ox-lbl">SESSION</div>
                <div class="ox-val">${yr}</div>
              </div>
              <div class="ox-data-item">
                <div class="ox-lbl">GRADE</div>
                <div class="ox-val">${grade}</div>
              </div>
              <div class="ox-data-item">
                <div class="ox-lbl">CAMPUS</div>
                <div class="ox-val">${DB.school.city || 'Karachi'}</div>
              </div>
            </div>
            
            <div class="ox-footer-bg"></div>
            
            <div class="ox-qr-wrap"><div class="ox-card-qr" data-student-id="${stu.id}"></div></div>
          </div>
        </div>
      </div>`;
    }
    ...
    function staffIdCardHtml(staff) {
      ...
      return `<div class="vid-card-wrap" style="display:inline-block; margin:10px;">
        <div class="vid-card">
          <div class="vid-card-inner ox-staff">
            <div class="ox-header-bg"></div>
            <div class="ox-logo-box"><img src="logo.png" onerror="this.src='https://cdn-icons-png.flaticon.com/512/4774/4774574.png'"></div>
            <div class="ox-logo-text">OXFORD EXCELLENCE ACADEMY</div>
            
            <div class="ox-photo-wrap">${staff.photo ? `<img src="${staff.photo}"/>` : `<span>${ini(staff.name||'?')}</span>`}</div>
            
            <div class="ox-name-wrap">
              <div class="ox-name">${(staff.name||'Staff').replace(/^(\S+)\s+(.+)$/, '$1<br>$2')}</div>
              <div class="ox-role">${isScanner ? 'SECURITY' : 'STAFF'}</div>
            </div>
            
            <div class="ox-grid">
              <div class="ox-data-item">
                <div class="ox-lbl">ID NO.</div>
                <div class="ox-val">STAFF</div>
              </div>
              <div class="ox-data-item">
                <div class="ox-lbl">SESSION</div>
                <div class="ox-val">${yr}</div>
              ...
              <div class="ox-data-item">
                <div class="ox-lbl">ROLE</div>
                <div class="ox-val">${staff.subject || (isScanner ? 'Gate Duty' : 'Teacher')}</div>
              </div>
              <div class="ox-data-item">
                <div class="ox-lbl">CAMPUS</div>
                <div class="ox-val">${DB.school.city || 'Karachi'}</div>
            ...
            <div class="ox-qr-wrap"><div class="ox-card-qr" data-staff-id="${staff.id}"></div></div>
          </div>
        </div>
      </div>`;
    }
...
<div class="id-print-area">
  ${sheets.length ? sheets.map(group => `<section class="id-a4-sheet">${group.map(e => isStaff ? staffIdCardHtml(e) : classIdCardHtml(e)).join('')}</section>`).join('') : '<div class="empty">No records found.</div>'}
    ...
    function buildClassIdCardQrs() {
      ...
      document.querySelectorAll('.ox-card-qr[data-student-id]').forEach(el => {
        const sid = el.dataset.studentId;
        const stu = allStus().find(s => String(s.id) === String(sid));
        const payload = stu ? studentAttendancePayload(stu) : JSON.stringify({ kind: 'school-student-v1', id: sid });
        el.innerHTML = '';
        try { new QRCode(el, { text: payload, width: 220, height: 220, colorDark: '#111827', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.L }); } catch { }
      });
      document.querySelectorAll('.ox-card-qr[data-staff-id]').forEach(el => {
        const tid = el.dataset.staffId;
        const staff = (DB.users || []).find(u => String(u.id) === String(tid));
        const payload = staff ? JSON.stringify({ kind: 'school-teacher-attendance-v1', id: staff.id }) : JSON.stringify({ kind: 'school-teacher-attendance-v1', id: tid });
        el.innerHTML = '';
        try { new QRCode(el, { text: payload, width: 220, height: 220, colorDark: '#111827', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.L }); } catch { }
      });
    }
...
<div class="vid-card-wrap fu d1">
      <div class="vid-card">
        <div class="vid-card-inner ${isTeacherCard ? 'ox-staff' : ''}">
            <div class="ox-header-bg"></div>
            <div class="ox-logo-box"><img src="logo.png" onerror="this.src='https://cdn-icons-png.flaticon.com/512/4774/4774574.png'"></div>
            <div class="ox-logo-text">OXFORD EXCELLENCE ACADEMY</div>
            
            <div class="ox-photo-wrap">${personPhoto ? `<img src="${personPhoto}"/>` : `<span>${ini(personName||'?')}</span>`}</div>
            
            <div class="ox-name-wrap">
              <div class="ox-name">${(personName||'Student').replace(/^(\S+)\s+(.+)$/, '$1<br>$2')}</div>
              <div class="ox-role">${isTeacherCard ? 'STAFF' : 'STUDENT'}</div>
            </div>
            
            <div class="ox-grid">
              <div class="ox-data-item">
                <div class="ox-lbl">ID NO.</div>
                <div class="ox-val">${isTeacherCard ? 'STAFF' : String(stu?.roll || '001').padStart(3,'0')}</div>
              </div>
              <div class="ox-data-item">
                <div class="ox-lbl">SESSION</div>
                <div class="ox-val">${DB.settings?.schoolYear || '2025–2026'}</div>
              </div>
              <div class="ox-data-item">
                <div class="ox-lbl">${isTeacherCard ? 'ROLE' : 'GRADE'}</div>
                <div class="ox-val">${chip}</div>
              </div>
              <div class="ox-data-item">
                <div class="ox-lbl">CAMPUS</div>
                <div class="ox-val">${DB.school.city || 'Karachi'}</div>
            ...
            <div class="ox-qr-wrap"><div class="ox-card-qr" id="qr-canvas-wrap"></div></div>
        </div>
      </div>
</div>
...
<div class="card fu d2">${DB.fees.items.map(f => { const isPaid = paid[f.id]?.paid; return `<div style="display:flex;align-items:center;gap:11px;padding:11px 0;border-bottom:1px solid var(--border);"><div style="font-size:1.1rem;">${isPaid ? '?' : '?'}</div><div style="flex:1;"><div style="font-weight:700;font-size:.82rem;">${f.name}</div><div style="font-size:.7rem;color:var(--ink3);">Due: ${f.due} - ${f.mandatory ? 'Mandatory' : 'Optional'}</div>${isPaid ? `<div style="font-size:.65rem;color:var(--g);margin-top:1px;">Paid on ${fmtDate(paid[f.id]?.date)}</div>` : ''}</div><div style="text-align:right;flex-shrink:0;"><div style="font-weight:800;color:${isPaid ? 'var(--g)' : 'var(--r)'};">${DB.settings?.currency || 'PKR'} ${Number(f.amount || 0).toLocaleString()}</div><span class="badge ${isPaid ? 'bg2' : 'br2'}">${isPaid ? 'Paid' : 'Pending'}</span></div></div>`; }).join('')}</div>`;
    ...
    function pgMyPayroll() {
      const t = curUser;
      window.mypMonth = window.mypMonth || monthKey();
      const p = calcPayrollData(t, window.mypMonth);
      const isPaid = (window.mypMonth === monthKey()) ? (t.payrollStatus === 'paid') : (t.lastPaid && t.lastPaid.startsWith(window.mypMonth));
      return `
<div class="ph ph-row fu">
  <div><div class="ph-tag"> My Payslips</div><h1>Payroll History</h1><div class="ph-sub">${t.name}</div></div>
  <div class="ph-acts">
    <input type="month" value="${window.mypMonth}" onchange="window.mypMonth=this.value; goPage('mypayroll')" style="padding:6px 10px; border-radius:8px; border:1px solid var(--border); font-size:0.8rem; background:var(--card); color:var(--ink); cursor:pointer;" />
  </div>
</div>
<div class="card fu d1">
  <div class="ch"><div class="ct">Payslip: ${window.mypMonth}</div></div>
  <div style="display:flex; justify-content:space-between; margin-bottom:12px; padding-bottom:12px; border-bottom:1px solid var(--border);">
    <div>Base Salary</div><div style="font-weight:700;">${fmtMoney(p.base)}</div>
  </div>
  <div style="display:flex; justify-content:space-between; margin-bottom:12px; padding-bottom:12px; border-bottom:1px solid var(--border); color:var(--r);">
    <div>Attendance Deduction (${p.deductionDays} days)</div><div style="font-weight:700;">-${fmtMoney(p.attDeduction)}</div>
  </div>
  <div style="display:flex; justify-content:space-between; margin-bottom:12px; padding-bottom:12px; border-bottom:1px solid var(--border); color:var(--p);">
    <div><b>Expected Net Salary</b></div><div style="font-size:1.2rem; font-weight:800;">${fmtMoney(p.expectedNet)}</div>
  </div>
  <div style="text-align:right; margin-top:20px;">
    ${isPaid ? `<span class="badge bg2" style="font-size:1rem; padding:8px 16px;">Paid</span>` : `<span class="badge bo2" style="font-size:1rem; padding:8px 16px;">Pending / Unpaid</span>`}
  </div>
</div>`;
    }
...
<div class="card fu d1">
  ...
  <button class="btn btn-gh" onclick="logout()" style="width:100%;justify-content:center;">Sign Out</button>
    ...
    function printIDCards(cid) {
      ...
      let cards = '';
      sts.forEach(s => {
        const qrPayload = encodeURIComponent(studentAttendancePayload(s));
        ...
        <div style="width:85.6mm;min-height:135mm;background:#fff;border-radius:14px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 4px 20px rgba(0,0,0,.18);page-break-inside:avoid;">
          ...
          <div style="flex:1;padding:14px 14px 10px;display:flex;gap:12px;align-items:flex-start;">
            ...
            <div style="flex:1;min-width:0;">
              ...
              <div style="display:flex;flex-direction:column;gap:5px;">
                ...
                <div style="display:flex;align-items:center;gap:6px;font-size:8px;border-bottom:1px solid #eee;padding-bottom:4px;">
                  <div style="width:18px;height:18px;border-radius:50%;background:${navy};display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:9px;"><span class="material-symbols-outlined" style="font-size:inherit;vertical-align:-2px;">school</span></div>
                ...
                <div style="display:flex;align-items:center;gap:6px;font-size:8px;border-bottom:1px solid #eee;padding-bottom:4px;">
                  <div style="width:18px;height:18px;border-radius:50%;background:${navy};display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:9px;"><span class="material-symbols-outlined" style="font-size:inherit;vertical-align:-2px;">calendar_month</span></div>
                ...
                <div style="display:flex;align-items:center;gap:6px;font-size:8px;">
                  <div style="width:18px;height:18px;border-radius:50%;background:${navy};display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:9px;"><span class="material-symbols-outlined" style="font-size:inherit;vertical-align:-2px;">location_on</span></div>
          ...
          <div style="padding:6px 14px 10px;display:flex;align-items:flex-end;justify-content:space-between;border-top:1px solid #f0f0f0;">
            <div class="idp-qr" data-student-id="${s.id}" data-qr-payload="${studentAttendancePayload(s)}" style="width:54px;height:54px;"></div>
            <div style="text-align:center;flex:1;padding-left:6px;">
              <img src="signature.jpg" style="width:75px;height:30px;object-fit:contain;display:block;margin:0 auto;" alt="Principal Signature"/>
              <div style="border-top:1px solid #bbb;margin-top:2px;padding-top:2px;font-size:6.5px;color:#888;letter-spacing:1.5px;font-weight:600;text-transform:uppercase;">PRINCIPAL</div>
            </div>
          </div>
          ...
          <div style="background:${navy};padding:5px 10px;display:flex;align-items:center;justify-content:center;gap:8px;">
            ...
            <span style="color:${gold};font-size:11px;"><span class="material-symbols-outlined" style="font-size:inherit;vertical-align:-2px;">security</span></span>
      ...
      <scr`+`ipt>
        function genQRs() {
          document.querySelectorAll('[data-qr-payload]').forEach(function(el) {
            var canvas = document.createElement('canvas');
            QRCode.toCanvas(canvas, el.getAttribute('data-qr-payload'), {width:108,margin:1,color:{dark:'#0d1e3d',light:'#ffffff'}}, function(){});
            el.appendChild(canvas);
          });
        }
        window.onload = function() {
          if (typeof QRCode !== 'undefined') { genQRs(); setTimeout(function(){window.print();}, 1400); }
          else { document.querySelector('script[src*="qrcode"]').onload = function(){ genQRs(); setTimeout(function(){window.print();}, 1400); }; }
        };
      <\/scr`+`ipt>
      </body></html>`;
    ...
    /* INIT handled after initDB() resolves above */function initScrollReveals() {
    ...
    // Automated Period Notifications for Teachers
    setInterval(() => {
        if (typeof curRole !== 'undefined' && curRole === 'teacher' && typeof DB !== 'undefined' && DB.periods && DB.timetable) {
            const now = new Date();
            ...
            DB.periods.forEach((p, pi) => {
                if (!p.time || !p.time.includes('-')) return;
                const startTimeStr = p.time.split('-')[0].trim();
                if (!startTimeStr.includes(':')) return;
                const [h, m] = startTimeStr.split(':').map(Number);
                const periodStartMins = h * 60 + m;
                
                // If period starts in exactly 5 minutes
                if (periodStartMins - currentMins === 5) {
                    DB.classes.forEach(c => {
                        let cell = DB.timetable[c.id]?.[dayName]?.[pi];
                        if (cell && cell.teacherId === curUser.id) {
                            showToast(`⏳ Reminder: Your ${cell.subject} class in ${c.name}${c.section?' '+c.section:''} starts in 5 minutes!`, 'ts', 15000);
                        }
                    });
                }
            });
