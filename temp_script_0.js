    ...
    'use strict';
    /* ------------------------------- SUPABASE CONFIG ------------------------------- */
    const SUPABASE_URL  = 'https://owojgzrzstigmumsehdw.supabase.co';
    const SUPABASE_KEY  = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im93b2pnenJ6c3RpZ211bXNlaGR3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzcwNDUwOTAsImV4cCI6MjA5MjYyMTA5MH0.5PLeyDfDBjO5pYaTc1RhfYSsXn3hbMh2tbSjBcfKLT0';
    const _sb = (typeof supabase !== 'undefined' && SUPABASE_KEY !== 'PASTE_YOUR_ANON_KEY_HERE')
    ...
    async function pushCloudDB() {
      ...
      try {
        await _sb.from('db_snapshot').upsert({ id: 1, data: DB, updated_at: new Date().toISOString() });
    ...
    function defaultDB() {
      return {
        school: { name: 'EduNova School', tagline: 'Playful learning, powerful management', logo: '', address: '', city: 'Karachi', phone: '', whatsapp: '', email: '', principal: '', primaryColor: '#ff8a00', founded: '', motto: '', website: '', heroText: 'A friendlier school website with complete management tools for staff and parents.' },
        admissions: [],
        classes: [
        ...
        timetable: {},
        users: [
          { id: 'u1', name: 'School Admin', email: 'admin@school.edu', role: 'admin', photo: '', phone: '', createdAt: '' },
    ...
    function avgStu(cid, sid) { const g = (DB.grades[cid] || {})[sid] || {}; const v = Object.values(g).filter(x => x > 0); return v.length ? Math.round(v.reduce((a, b) => a + b, 0) / v.length) : 0; }
    ...
    function teacherNetSalary(t) { return Math.max(0, num(t.salary) - num(t.deductions) + num(t.bonus)); }
    function teacherMonthlyStats(id) {
    ...
    async function doLogin() {
      ...
      if (Date.now() < _loginLockUntil) { showToast('Too many attempts. Wait ' + Math.ceil((_loginLockUntil-Date.now())/1000) + 's', 'te', 3000); return; }
      _loginAttempts++;
      const btn = document.querySelector('.login-btn');
      if (btn) { btn.disabled = true; btn.textContent = 'Signing in...'; }
      try {
        const { error } = await _sb.auth.signInWithPassword({ email: em, password: pw });
        if (error) { if (_loginAttempts >= 5) { _loginLockUntil = Date.now() + 60000; _loginAttempts = 0; showToast('Too many failed attempts. Locked 60s.','te',5200); } else { showToast(error.message||'Incorrect email or password','te',5200); } return; }
        _loginAttempts = 0;
        const cloud = await loadCloudDB();
        if (cloud) DB = cloud;
        stripStoredPasswords();
        ...
        if (!u) { await _sb.auth.signOut(); showToast('Signed in, but this email is not linked to a teacher, student, parent, or admin profile.', 'te', 5200); return; }
    ...
    function slideStyle(slide, i) {
      ...
      const fallbackImages = [
        'https://images.unsplash.com/photo-1509062522246-3755977927d7?auto=format&fit=crop&w=1200&q=70',
    ...
    function renderLandingSlides() {
      ...
      const dots = document.getElementById('hs-dots');
      if (!wrap || !dots || !DB?.school) return;
      const slides = (DB.school.slides && DB.school.slides.length ? DB.school.slides : defaultSlides()).slice(0, 4);
      wrap.innerHTML = slides.map((slide, i) => '<div class="hs-slide ' + (i === 0 ? 'active' : '') + '" style="' + slideStyle(slide, i) + '"><div class="hs-overlay"><div class="hs-tag">' + (slide.icon || '') + ' ' + (slide.tag || '') + '</div><h2 class="hs-title">' + (slide.title || DB.school.name || 'Our School') + '</h2><p class="hs-sub">' + (slide.text || DB.school.tagline || '') + '</p><div class="hs-btns"><button class="btn btn-p" onclick="openLogin()">School Portal</button><button class="btn btn-gh" onclick="document.getElementById(\'land-contact\').scrollIntoView({behavior:\'smooth\'})">Contact Us</button></div></div><div class="hs-deco">' + (slide.icon || '') + '</div></div>').join('');
    ...
    function applyBranding() {
      ...
      setText('doc-title', s.name || 'EduNova');
      setText('tb-sname', s.name || 'EduNova');
      setText('lp-title', s.name || 'EduNova');
      setText('lp-sub', s.tagline || 'School Management System');
      setText('land-name', s.name || 'EduNova');
      setText('land-copy', s.tagline || 'School Management System');
      const landHero = document.getElementById('land-hero-text'); if (landHero) landHero.textContent = s.heroText || 'A friendlier school website with complete management tools for staff and parents.';
      const studentStat = document.getElementById('land-stat-students'); if (studentStat) studentStat.textContent = `${allStus().length}+`;
      const teacherStat = document.getElementById('land-stat-teachers'); if (teacherStat) teacherStat.textContent = `${DB.users.filter(u => u.role === 'teacher').length}+`;
      const classStat = document.getElementById('land-stat-classes'); if (classStat) classStat.textContent = `${DB.classes.length}+`;
      ['tb-ico', 'lp-logo', 'land-mark'].forEach(id => {
        const el = document.getElementById(id); if (!el) return;
        el.innerHTML = s.logo ? `<img src="${s.logo}" alt="logo"/>` : '';
        el.style.background = s.logo ? 'transparent' : 'linear-gradient(135deg,' + (s.primaryColor || '#ff8a00') + ',var(--p2))';
      });
  ...
  <button class="sb-out" onclick="logout()"><span class="ni-ic">${iconMarkup('logout')}</span> Sign Out</button>`;
    ...
    function goPage(p) {
      ...
      updatePill();
      closeNotif();
      cleanBrokenUI(document.getElementById('page-root'));
    ...
    function renderPage(p) {
      const map = {
        dashboard: () => curRole === 'admin' ? pgAdminDash() : curRole === 'teacher' ? pgTeacherDash() : pgStudentDash(),
...
<div class="g2">
  <div class="card fu d2">
    <div class="ch"><div class="ct"> Class Performance</div><button class="btn btn-gh btn-sm" onclick="goPage('reportcards')">Reports</button></div>
      ${DB.classes.slice(0, 8).map(c => { const sts = clsStus(c.id); const avg = sts.length ? Math.round(sts.reduce((a, s) => a + avgStu(c.id, s.id), 0) / sts.length) : 0; return `<div style="display:flex;align-items:center;gap:9px;margin-bottom:9px;"><div style="font-size:.74rem;font-weight:600;color:var(--ink3);min-width:78px;">${c.name}${c.section ? ' ' + c.section : ''}</div><div style="flex:1;"><div class="pb" style="height:6px;"><div class="pf" style="width:${avg}%;background:${sCol(avg)};height:6px;"></div></div></div><div style="font-weight:700;font-size:.78rem;min-width:34px;text-align:right;color:${sCol(avg)};">${avg}%</div></div>`; }).join('')}
  </div>
  <div>
...
<div class="g2">
  <div class="card fu d2">
    <div class="ch"><div class="ct"> Subject Scores</div><button class="btn btn-gh btn-sm" onclick="goPage('mygrades')">Full</button></div>
    <div class="score-mini">${DB.subjects.map(sub => { const v = (DB.grades[cid]?.[stu.id] || {})[sub.id] || 0; return `<div class="sm-row"><div class="sm-lbl">${sub.icon} ${sub.name}</div><div class="sm-bar"><div class="sm-fill" style="width:${v}%;background:${sub.color};"></div></div><div class="sm-val" style="color:${sub.color};">${v || '-'}</div></div>`; }).join('')}</div>
  </div>
  <div>
...
<div class="card fu d2" style="padding:0;overflow:hidden;">
  <div class="tw">
    <table>
      <thead><tr><th>#</th><th>Photo</th><th>Name</th><th>Roll No.</th><th>Gender</th><th>Avg</th><th>Att%</th><th>Monthly Fee</th><th>Fees</th><th>Login</th><th>Actions</th></tr></thead>
      <tbody>
        ...
        const av = avgStu(curCls, s.id); const ap = attPct(curCls, s.id); const fam = s.familyId ? DB.families[s.familyId] : null; return `<tr>
          ...
          <td>${feeBadgeHtml(s)}${s.feeNote ? `<div style='font-size:.6rem;color:var(--ink4);margin-top:2px;'>${s.feeNote}</div>` : ''}</td>
...
<div class="card fu">
  <div class="ch"><div class="ct">Applications</div><button class="btn btn-gh btn-sm" onclick="refreshAdmissionApplications()">Refresh Online</button></div>
    ...
    async function refreshAdmissionApplications() {
      ...
      if (error) {
        ...
        showToast('Admission table not ready in Supabase', 'te');
        return;
      }
  ...
  <div class="fg2">
    <div class="fg"><label class="fl">Fee Type</label>
      <select class="fi" id="m-feetype" onchange="updateFeePreview()">
        <option value="default" ${(!s?.feeType||s?.feeType==='default')?'selected':''}>Default (Class Fee)</option>
        <option value="custom" ${s?.feeType==='custom'?'selected':''}>Custom Amount</option>
        <option value="discount" ${s?.feeType==='discount'?'selected':''}>Partial Discount %</option>
        <option value="scholarship" ${s?.feeType==='scholarship'?'selected':''}>Full Scholarship (Free)</option>
      </select>
    </div>
    <div class="fg" id="m-fee-amount-wrap" style="${s?.feeType==='scholarship'?'opacity:.4;pointer-events:none;':''}">
      <label class="fl" id="m-fee-label">${s?.feeType==='discount'?'Discount %':'Monthly Fee (PKR)'}</label>
  ...
  <div class="fg"><label class="fl">Login Email</label><input class="fi" id="m-lemail" value="${s?.loginEmail || ''}" placeholder="student@email.com"/></div>
  <div class="div"></div>
  ...
  <div class="m-btns">
    ...
    function updateFeePreview() {
      var type = (document.getElementById('m-feetype')||{}).value || 'default';
      var wrap = document.getElementById('m-fee-amount-wrap');
      var lbl = document.getElementById('m-fee-label');
      var inp = document.getElementById('m-customfee');
      var prev = document.getElementById('m-fee-preview');
      var base = num(((DB.fees.items||[]).find(function(f){return f.id==='f1'})||{}).amount||0);
      var cur = (DB.settings||{}).currency || 'PKR';
      if (wrap) { wrap.style.opacity = type==='scholarship'?'.4':'1'; wrap.style.pointerEvents = type==='scholarship'?'none':''; }
      if (lbl) lbl.textContent = type==='discount'?'Discount %':'Monthly Fee ('+cur+')';
    ...
    function saveStu(editId) {
      ...
      const data = { name, roll, classId, gender: document.getElementById('m-gender')?.value || '', dob: document.getElementById('m-dob')?.value || '', blood: document.getElementById('m-blood')?.value || '', fees: document.getElementById('m-fees')?.value || 'Pending', father: (document.getElementById('m-father')?.value || '').trim(), fphone: (document.getElementById('m-fphone')?.value || '').trim(), mother: (document.getElementById('m-mother')?.value || '').trim(), address: (document.getElementById('m-addr')?.value || '').trim(), whatsapp: wa, photo, familyId, loginEmail, customFee: (function(){var t=(document.getElementById('m-feetype')||{}).value||'default';return t==='custom'?num((document.getElementById('m-customfee')||{}).value||0):0;})(), feeType: (document.getElementById('m-feetype')||{}).value||'default', feeDiscount: (function(){var t=(document.getElementById('m-feetype')||{}).value||'default';return t==='discount'?num((document.getElementById('m-customfee')||{}).value||0):0;})(), feeNote: ((document.getElementById('m-feenote')||{}).value||'').trim() };
      ...
      if (editId) {
        ...
        showToast(`Student added. Create/invite ${loginEmail} in Supabase Auth.`, 'ts');
      }
    ...
    function delStu(sid, cid) {
      ...
      delete (DB.grades[cid] || {})[sid]; delete (DB.attendance[cid] || {})[sid]; delete DB.fees.payments[sid];
  ...
  <div style="font-size:.76rem;font-weight:700;color:var(--ink2);margin-bottom:8px;">Subject Scores</div>
  <div class="score-mini">${DB.subjects.map(sub => { const v = (DB.grades[cid]?.[sid] || {})[sub.id] || 0; return `<div class="sm-row"><div class="sm-lbl">${sub.icon} ${sub.name}</div><div class="sm-bar"><div class="sm-fill" style="width:${v}%;background:${sub.color};"></div></div><div class="sm-val" style="color:${sub.color};">${v || '-'}</div></div>`; }).join('')}</div>
  ...
  <div class="m-btns">
    ...
    function pgAttendance() {
      const sts = clsStus(curCls); const cls = DB.classes.find(c => c.id === curCls);
      const dates = []; for (let i = 9; i >= 0; i--) { const d = new Date(); d.setDate(d.getDate() - i); dates.push(d.toISOString().split('T')[0]); }
      return `
<div class="ph ph-row fu">
  ...
  <button class="btn btn-wa btn-sm" style="margin-left:auto;" onclick="sendAbsentAlerts()"> Absent Alerts</button>
    ...
    function pgTeacherAtt() {
      ...
      if (!isAdmin) {
        ...
        const entries = Object.entries(myRec).sort((a, b) => b[0].localeCompare(a[0]));
...
<div class="ph ph-row fu">
  ...
  <div class="ph-acts"><button class="btn btn-gh btn-sm" onclick="clearGrades()">Clear</button><button class="btn btn-g btn-sm" onclick="saveGrades()"> Save Grades</button></div>
</div>
  ${clsBar('switchGrdCls')}
...
<div class="card fu d2" style="padding:0;overflow:hidden;"><div class="tw"><table>
  ...
  <tbody>${sts.map(s => {
        if (!DB.grades[curCls]) DB.grades[curCls] = {};
        if (!DB.grades[curCls][s.id]) DB.grades[curCls][s.id] = {};
        const g = DB.grades[curCls][s.id];
    ...
    function setGrade(sid, subId, val, inp) {
      if (!DB.grades[curCls]) DB.grades[curCls] = {};
      if (!DB.grades[curCls][sid]) DB.grades[curCls][sid] = {};
      const v = Math.min(100, Math.max(0, parseInt(val) || 0));
      DB.grades[curCls][sid][subId] = v;
      const vals = Object.values(DB.grades[curCls][sid]).filter(x => x > 0);
    ...
    function clearGrades() { if (!confirm('Clear all grades for this class?')) return; delete DB.grades[curCls]; saveDB(); goPage('grades'); }
    ...
    function studentFeeSummary(sid) {
      ...
      const itemAmount = f => (f.id === 'f1' && num(stu?.customFee)) ? num(stu.customFee) : num(f.amount);
      const mandatory = DB.fees.items.filter(f => f.mandatory);
      const allItems = DB.fees.items || [];
      const paidAmount = allItems.reduce((sum, f) => sum + (f.id === 'f1' ? monthlyFeePaidTotal(sid) : (payments[f.id]?.paid ? num(payments[f.id].amount || itemAmount(f)) : 0)), 0);
      const dueAmount = allItems.reduce((sum, f) => sum + (f.id === 'f1' ? monthlyFeeDue(sid) : (!payments[f.id]?.paid ? itemAmount(f) : 0)), 0);
...
<div class="fee-hero fu">
  <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;">
    ...
    <div style="text-align:right;"><div style="font-size:.7rem;opacity:.65;">Pending Balance</div><div style="font-family:var(--head);font-size:1.1rem;font-weight:700;margin-top:3px;">${fmtMoney(pending)}</div><div style="font-size:.68rem;opacity:.62;">Mandatory/year ${fmtMoney(totalAnnual)}</div></div>
  </div>
...
<div class="g2">
  ...
  <div class="card fu d3" style="padding:0;overflow:hidden;">
    ...
    function toggleFee(sid, fid, stayOpen = false) {
      if (!DB.fees.payments[sid]) DB.fees.payments[sid] = {};
      const fee = DB.fees.items.find(f => f.id === fid) || { amount: 0 };
      if (fid === 'f1') {
        const dueBefore = monthlyFeeDue(sid);
        if (dueBefore > 0) markMonthlyFeePaid(sid);
        else {
          const row = DB.fees.monthlyLedger?.[sid]?.[monthKey()];
          if (row) { row.paid = false; delete row.date; }
          DB.fees.payments[sid][fid] = { paid: false, date: '', amount: monthlyFeeAmount(allStus().find(s => s.id === sid)), month: monthKey() };
        }
      } else {
      ...
      saveDB();
      if (stayOpen) openStudentFeeModal(sid); else goPage('fees');
      showToast(DB.fees.payments[sid][fid].paid ? 'Marked paid' : 'Unmarked');
    }
    function openStudentFeeModal(sid) {
  ...
  <div class="m-btns" style="flex-wrap:wrap;">
    ...
    function printFeeReceipt(sid, fid) {
      const s = allStus().find(x => x.id === sid); if (!s) return;
      const fee = DB.fees.items.find(f => f.id === fid) || DB.fees.items[0]; if(!fee) return;
      const sum = studentFeeSummary(sid);
      const paidObj = sum.payments[fid]; if (!paidObj?.paid) return showToast('Fee is not marked paid yet.', 'te');
      const cls = DB.classes.find(c => c.id === s.classId);
      const school = DB.school || { name: 'My School', address: '', phone: '' };
      const cur = DB.settings?.currency || 'PKR';
      const amount = (fid === 'f1' ? (monthlyFeeDue(sid) || monthlyFeeAmount(s)) : num(fee.amount));
      ...
      const fullHtml = receiptHtml('School Copy') + '<div class="print-divider"></div>' + receiptHtml('Parent Copy');
      document.getElementById('print-container').innerHTML = fullHtml;
    ...
    function feeReminderText(sid, fid, dueDate, note = '') {
      ...
      const cls = DB.classes.find(c => c.id === s.classId);
      const amount = (fid === 'f1' && num(s.customFee)) ? num(s.customFee) : num(fee.amount);
  ...
  <div class="m-btns">
      ${current ? '<button class="btn btn-wa" onclick="openNextBulkFeeReminder()">Open Next WhatsApp</button>' : ''}
...
<div class="g3 fu d1">${DB.classes.map(c => { const sts = clsStus(c.id); const avg = sts.length ? Math.round(sts.reduce((a, s) => a + avgStu(c.id, s.id), 0) / sts.length) : 0; return `<div class="card" style="cursor:pointer;padding:13px;transition:all .17s;" onclick="switchCls('${c.id}');goPage('students')" onmouseover="this.style.transform='translateY(-2px)';this.style.borderColor='var(--p2)'" onmouseout="this.style.transform='';this.style.borderColor=''"><div style="display:flex;justify-content:space-between;margin-bottom:8px;"><div style="width:28px;height:28px;border-radius:7px;background:${gc(c.name)};display:flex;align-items:center;justify-content:center;font-size:.7rem;font-weight:800;color:#fff;">${c.name.charAt(0)}</div><button class="btn btn-xs btn-gh" onclick="event.stopPropagation();openAddClsModal('${c.id}')">Edit</button></div><div style="font-weight:700;font-size:.85rem;">${c.name}${c.section ? ' ' + c.section : ''}</div><div style="font-size:.68rem;color:var(--ink3);margin:2px 0 8px;">${sts.length} students - ${c.level === 'primary' ? 'Primary' : 'Secondary'}</div><div class="pb" style="height:4px;"><div class="pf" style="width:${avg}%;background:${sCol(avg)};height:4px;"></div></div><div style="display:flex;justify-content:space-between;margin-top:4px;font-size:.66rem;font-weight:700;"><span style="color:${sCol(avg)};">${avg}% avg</span><span style="color:var(--p);">View</span></div></div>`; }).join('')}<div class="card" style="display:flex;align-items:center;justify-content:center;border:2px dashed var(--border2);background:transparent;cursor:pointer;min-height:120px;" onclick="openAddClsModal()"><div style="text-align:center;color:var(--ink4);"><div style="font-size:1.3rem;">+</div><div style="font-size:.74rem;font-weight:600;margin-top:3px;">Add Class</div></div></div></div>`;
    }
  ...
  <div class="m-btns"><button class="btn btn-p" onclick="saveCls('${eid || ''}')"> ${c ? 'Update' : 'Add'}</button>${c ? `<button class="btn btn-xs" style="background:var(--rl);color:var(--r);" onclick="delCls('${c.id}')">Delete</button>` : ''}<button class="btn btn-gh" onclick="closeModal()">Cancel</button></div>`);
    ...
    function delCls(id) { if (!confirm('Delete class and all students?')) return; DB.classes = DB.classes.filter(c => c.id !== id); delete DB.students[id]; delete DB.grades[id]; delete DB.attendance[id]; saveDB(); closeModal(); showToast('Deleted'); goPage('classes'); }
...
<div class="card fu d2" style="padding:0;overflow:hidden;"><div class="tw"><table style="min-width:480px;">
  ...
  <tbody>${PERIODS.map((p, pi) => `<tr><td style="font-weight:700;font-size:.72rem;background:var(--card2);color:var(--ink3);white-space:nowrap;">${p}</td>${p === 'Break' ? `<td colspan="${DAYS.length}" style="background:var(--card3);text-align:center;font-size:.72rem;font-weight:700;color:var(--ink4);">? Break</td>` : DAYS.map(d => { const v = tt[d]?.[pi] || ''; return `<td style="padding:4px;"><select class="fi" style="font-size:.7rem;padding:5px 8px;min-width:90px;" onchange="setTT('${d}',${pi},this.value)"><option value="">-</option>${DB.subjects.map(s => `<option value="${s.name}" ${v === s.name ? 'selected' : ''}>${s.icon} ${s.name}</option>`).join('')}</select></td>`; }).join('')}</tr>`).join('')}
    ...
    function pgTeachers() {
      const teachers = DB.users.filter(u => u.role === 'teacher');
      return `
<div class="ph ph-row fu"><div><div class="ph-tag"> Teachers</div><h1>Teaching Staff</h1></div><button class="btn btn-p" onclick="openTeachModal()">+ Add Teacher</button></div>
...
<div class="card fu d1" style="padding:0;overflow:hidden;"><div class="tw"><table>
  ...
  <div class="ibox" style="margin-bottom:10px;">Create or invite this teacher email in Supabase Auth. Passwords are not stored here.</div><div class="fg"><label class="fl">Main Subject</label><select class="fi" id="mt-sub"><option value="">Select</option>${DB.subjects.map(s => `<option value="${s.name}" ${t?.subject === s.name ? 'selected' : ''}>${s.name}</option>`).join('')}</select></div></div>
  ...
  <div class="fg2"><div class="fg"><label class="fl">Base Salary</label><input class="fi" type="number" id="mt-salary" value="${num(t?.salary || 40000)}" placeholder="45000"/></div><div class="fg"><label class="fl">Bonus</label><input class="fi" type="number" id="mt-bonus" value="${num(t?.bonus || 0)}" placeholder="0"/></div></div>
  <div class="fg2"><div class="fg"><label class="fl">Deductions</label><input class="fi" type="number" id="mt-deductions" value="${num(t?.deductions || 0)}" placeholder="0"/></div><div class="fg"><label class="fl">Payroll Status</label><select class="fi" id="mt-paystatus"><option value="pending" ${t?.payrollStatus !== 'paid' ? 'selected' : ''}>Pending</option><option value="paid" ${t?.payrollStatus === 'paid' ? 'selected' : ''}>Paid</option></select></div></div>
  <div class="fg"><label class="fl">Notes</label><textarea class="fi" id="mt-notes" style="min-height:70px;" placeholder="Optional notes">${t?.notes || ''}</textarea></div>
  ...
  <div class="m-btns"><button class="btn btn-p" onclick="saveTeach('${eid || ''}')"> ${t ? 'Update' : 'Add'} Teacher</button><button class="btn btn-gh" onclick="closeModal()">Cancel</button></div>`);
    }
    function saveTeach(eid) { const name = (document.getElementById('mt-name')?.value || '').trim(); const email = (document.getElementById('mt-email')?.value || '').trim().toLowerCase(); if (!name || !email) { showToast('Name & email required', 'te'); return; } const classIds = [...document.querySelectorAll('#modal-body input[type=checkbox]:checked')].map(x => x.value); const obj = { name, email, subject: document.getElementById('mt-sub')?.value || '', classIds, role: 'teacher', phone: document.getElementById('mt-phone')?.value?.trim() || '', joinDate: document.getElementById('mt-join')?.value || '', salary: num(document.getElementById('mt-salary')?.value || 0), bonus: num(document.getElementById('mt-bonus')?.value || 0), deductions: num(document.getElementById('mt-deductions')?.value || 0), payrollStatus: document.getElementById('mt-paystatus')?.value || 'pending', notes: document.getElementById('mt-notes')?.value?.trim() || '' }; if (eid) { const t = DB.users.find(x => x.id === eid); if (t) Object.assign(t, obj); } else DB.users.push({ id: uid(), ...obj, photo: '', lastPaid: '', payrollHistory: [] }); saveDB(); pushCloudDB(); closeModal(); showToast('Teacher saved', 'ts'); goPage('teachers'); }

    function pgPayroll() {
      const teachers = DB.users.filter(u => u.role === 'teacher');
      const total = teachers.reduce((sum, t) => sum + teacherNetSalary(t), 0);
      const paid = teachers.filter(t => t.payrollStatus === 'paid').length;
      return `
<div class="ph ph-row fu"><div><div class="ph-tag"> Payroll</div><h1>Teacher Salary System</h1><div class="ph-sub">Manage salary, bonus, deductions, and payment status</div></div><button class="btn btn-p" onclick="openTeachModal()">+ Add Teacher</button></div>
<div class="sg fu d1">
  <div class="sc sp"><div class="sc-ic"></div><div class="sc-val" style="color:var(--p);">${fmtMoney(total)}</div><div class="sc-lbl">Monthly Payroll</div></div>
  ...
  ${teachers.length === 0 ? `<div class="card"><div class="empty"><div class="empty-ic"></div><div class="empty-t">No teacher payroll records yet</div><button class="btn btn-p btn-sm" style="margin-top:10px;" onclick="openTeachModal()">Add Teacher</button></div></div>` :
    ...
    function printPayslip(id) {
      ...
      const stats = teacherMonthlyStats(id);
      const mName = t.payrollMonth || monthKey();
      const school = DB.school || { name: 'My School', address: '', phone: '' };
      const net = teacherNetSalary(t);
      const cur = DB.settings?.currency || 'PKR';
      ...
      const fullHtml = slipHtml('Office Copy') + '<div class="print-divider"></div>' + slipHtml('Teacher Copy');
      document.getElementById('print-container').innerHTML = fullHtml;
    ...
    function payTeacherSalary(id) {
      const t = DB.users.find(u => u.id === id && u.role === 'teacher'); if (!t) return;
      const amount = teacherNetSalary(t);
      if (!confirm(`Mark ${fmtMoney(amount)} as paid to ${t.name}?`)) return;
      t.payrollStatus = 'paid';
      t.payrollMonth = monthKey();
      t.lastPaid = today();
      t.payrollHistory = t.payrollHistory || [];
      const existing = t.payrollHistory.find(h => h.month === t.payrollMonth);
      const rec = { id: uid(), month: t.payrollMonth, paidOn: today(), amount, base: num(t.salary), bonus: num(t.bonus), deductions: num(t.deductions) };
      if (existing) Object.assign(existing, rec); else t.payrollHistory.push(rec);
      saveDB();
      showToast(`Salary paid to ${t.name}`, 'ts');
      goPage('payroll');
    }
...
<div class="ph ph-row fu"><div><div class="ph-tag"> Report Cards</div><h1>Print Report Cards</h1><div class="ph-sub">Select a student to generate & print</div></div>
  <button class="btn btn-gh btn-sm no-print" onclick="window.print()"> Print</button></div>
  ${clsBar('switchRCCls')}
...
<div class="card fu d1"><div class="ch"><div class="ct">Select Student</div></div><div style="display:flex;flex-wrap:wrap;gap:7px;">${sts.map(s => `<button class="btn btn-gh btn-sm" onclick="openReportCard('${s.id}','${curCls}')" style="display:flex;align-items:center;gap:7px;"><div class="ava" style="width:22px;height:22px;border-radius:5px;background:${gc(s.name)};font-size:.46rem;flex-shrink:0;">${s.photo ? `<img src="${s.photo}" style="width:100%;height:100%;object-fit:cover;border-radius:5px;"/>` : ini(s.name)}</div>${s.name}</button>`).join('')}</div></div>`}`;
    ...
    function openReportCard(sid, cid) {
      const s = clsStus(cid).find(x => x.id === sid); if (!s) return;
      const cls = DB.classes.find(c => c.id === cid);
      const grades = DB.grades[cid]?.[sid] || {};
...
<div class="ph ph-row fu"><div><div class="ph-tag"> Families</div><h1>Family Management</h1><div class="ph-sub">Link siblings under one parent login</div></div><button class="btn btn-p" onclick="openFamModal()">+ Add Family</button></div>
  ...
  <div class="ibox" style="margin-bottom:10px;">Create or invite the parent email in Supabase Auth. Passwords are not stored here.</div>
...
<div class="card fu d1">
  ...
  <div style="display:flex;align-items:center;gap:14px;margin-bottom:15px;flex-wrap:wrap;">
    <div class="se-logo" onclick="document.getElementById('logo-up2').click()" title="Click to change logo">
        ${s.logo ? `<img src="${s.logo}" style="width:100%;height:100%;object-fit:cover;border-radius:14px;"/>` : ''}
    </div>
...
<div class="card fu d6">
  ...
  <div class="ibox" style="margin-top:10px;"> Data is saved to Supabase cloud + local browser cache.</div>
    ...
    function exportData() { const blob = new Blob([JSON.stringify(DB, null, 2)], { type: 'application/json' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `edunova-backup-${today()}.json`; a.click(); showToast('? Exported', 'ts'); }
    ...
    function pgMyGrades() {
      ...
      const grades = DB.grades[cid]?.[stu.id] || {}; const av = avgStu(cid, stu.id);
      return `
<div class="ph fu"><div class="ph-tag"> My Grades</div><h1>${stu.name || curUser?.name}</h1><div class="ph-sub">${(DB.classes.find(c => c.id === cid) || {}).name || ''} - Roll ${stu.roll || '-'}</div></div>
...
<div class="card fu d2"><div class="ch"><div class="ct">Attendance Log</div></div>
    ...
    function classIdCardHtml(stu) {
      ...
      return `<div class="school-id-card">
        <div class="idp-brand">
          <div class="idp-logo">${DB.school.logo ? `<img src="${DB.school.logo}"/>` : (DB.school.name || 'S').slice(0, 1).toUpperCase()}</div>
          <div class="idp-school">${DB.school.name || 'School'}</div>
        </div>
        <div class="idp-photo">${stu.photo ? `<img src="${stu.photo}"/>` : ini(stu.name || '?')}</div>
        <div class="idp-main">
          <div class="idp-name">${stu.name || 'Student'}</div>
          <div class="idp-role">Student</div>
          <div class="idp-row"><span>ID Number</span><b>${idNo || '-'}</b></div>
          <div class="idp-row"><span>Grade</span><b>${grade}</b></div>
        </div>
        <div class="idp-qr" data-student-id="${stu.id}"></div>
        <div class="idp-footer">${DB.school.city || ''}${DB.school.phone ? ' | ' + DB.school.phone : ''}</div>
      </div>`;
    }
    ...
    function pgClassIdCards() {
      ...
      const cls = DB.classes.find(c => c.id === curCls) || DB.classes[0];
      if (cls && cls.id !== curCls) curCls = cls.id;
      const students = cls ? clsStus(cls.id) : [];
      const perPage = 10;
      const sheets = [];
      for (let i = 0; i < students.length; i += perPage) sheets.push(students.slice(i, i + perPage));
      return `
<div class="ph ph-row fu no-print"><div><div class="ph-tag">ID Cards</div><h1>Class ID Cards</h1><div class="ph-sub">Print ${students.length} attendance QR cards for ${cls?.name || 'selected class'} on A4 sheets.</div></div>
  <div class="ph-acts"><select class="fi" style="width:auto;min-width:180px;" onchange="switchIdCardClass(this.value)">${DB.classes.map(c => `<option value="${c.id}" ${c.id === curCls ? 'selected' : ''}>${c.name}${c.section ? ' ' + c.section : ''}</option>`).join('')}</select><button class="btn btn-p btn-sm" onclick="printClassIdCards()">Print A4 Cards</button></div></div>
<div class="id-print-toolbar no-print"><div class="ibox" style="margin:0;flex:1;">Each card is 3.375in x 2.125in and includes the attendance QR used by the scanner.</div></div>
      <div id="print-container"></div>
      <div class="id-print-area">
  ${sheets.length ? sheets.map(group => `<section class="id-a4-sheet">${group.map(classIdCardHtml).join('')}</section>`).join('') : '<div class="empty">No students found in this class.</div>'}
</div>`;
    }
    ...
    function buildClassIdCardQrs() {
      ...
      document.querySelectorAll('.idp-qr[data-student-id]').forEach(el => {
        ...
        if (!stu) return;
        el.innerHTML = '';
        try { new QRCode(el, { text: studentAttendancePayload(stu), width: 72, height: 72, colorDark: '#111827', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.M }); } catch { }
      });
    }
    function pgQRCard() {
...
<div class="id-card-wrap fu d1">
  <div class="id-card">
    <div class="id-card-header">
      <div class="id-card-logo">${DB.school.logo ? `<img src="${DB.school.logo}"/>` : ''}</div>
      <div class="id-card-school">${DB.school.name}<br/><span style="font-weight:400;opacity:.65;">${DB.school.city || ''}</span></div>
    </div>
    <div class="id-card-body">
      <div class="id-card-photo">${personPhoto ? `<img src="${personPhoto}"/>` : ini(personName || '?')}</div>
      <div class="id-card-info">
        <div class="id-card-name">${personName}</div>
        <div class="id-card-roll">${isTeacherCard ? 'Staff ID' : 'Roll No. ' + (stu?.roll || '-')}</div>
        <div class="id-card-chip">${isTeacherCard ? '' : ''} ${chip}</div><br/>
          ${!isTeacherCard && stu?.dob ? `<div style="font-size:.62rem;opacity:.5;margin-top:3px;">DOB: ${fmtDate(stu.dob)}</div>` : ''}
          ${!isTeacherCard && stu?.blood ? `<div class="id-card-chip" style="margin-top:4px;"> ${stu.blood}</div>` : ''}
      </div>
    </div>
    <div class="id-card-qr">
      <div class="qr-wrap" id="qr-canvas-wrap"></div>
    </div>
    <div class="id-card-footer">
      <span>${DB.settings?.schoolYear || ''}</span>
      <span>ID: ${((isTeacherCard ? curUser?.id : stu?.id) || '').slice(-6).toUpperCase()}</span>
...
<div class="card fu d1">
  ...
  <button class="btn btn-gh" onclick="logout()" style="width:100%;justify-content:center;">Sign Out</button>
    ...
    function updateProfilePhoto(inp) { const f = inp.files[0]; if (!f) return; const r = new FileReader(); r.onload = e => { curUser.photo = e.target.result; const u = DB.users.find(x => x.id === curUser.id); if (u) u.photo = e.target.result; saveDB(); buildShell(); showToast('? Photo updated', 'ts'); }; r.readAsDataURL(f); }
    ...
    function onQRScan(raw) {
      ...
      try {
        ...
        if (data.kind === 'school-teacher-attendance-v1' || data.role === 'teacher') {
          if (curRole !== 'admin') throw new Error('Only gate/admin users can mark teacher attendance');
    ...
    /* ------------------------------- MODAL / TOAST / NOTIF ------------------------------- */
    function openModal(html) { const body = document.getElementById('modal-body'); body.innerHTML = html; cleanBrokenUI(body); document.getElementById('modal-bg').classList.add('open'); }
