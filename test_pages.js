  ...
  <div>
    ...
    <div class="card fu d3">
      <div class="ch"><div class="ct"> Teacher Payroll</div><button class="btn btn-gh btn-sm" onclick="goPage('payroll')">Open</button></div>
        ${teachers.slice(0, 3).map(t => `<div style="display:flex;justify-content:space-between;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);"><div><div style="font-weight:800;font-size:.8rem;">${t.name}</div><div style="font-size:.68rem;color:var(--ink4);">${t.subject || 'General'} - ${t.payrollStatus === 'paid' && t.lastPaid ? `Paid ${fmtDate(t.lastPaid)}` : 'Awaiting salary release'}</div></div><div style="text-align:right;"><div style="font-weight:800;color:var(--p);">${fmtMoney(calcPayrollData(t, monthKey()).netSalary)}</div><span class="badge ${t.payrollStatus === 'paid' ? 'bg2' : 'bo2'}">${t.payrollStatus === 'paid' ? 'Paid' : 'Pending'}</span></div></div>`).join('') || '<div style="font-size:.76rem;color:var(--ink4);">No teachers added yet</div>'}
    </div>
  ...
  <tbody>${sts.map(s => {
    ...
    function studentFeeSummary(sid) {
      ...
      if (DB.fees.monthlyLedger && DB.fees.monthlyLedger[sid]) {
         Object.values(DB.fees.monthlyLedger[sid]).forEach(m => paidAmount += num(m.amountPaid));
      }
      
      const dueAmount = num(stu?.arrears || 0);
      const currentMonth = monthKey();
      const currentBill = typeof calcStudentFeeBill === 'function' ? calcStudentFeeBill(sid, currentMonth) : null;
      let totalDue = dueAmount;
      if (currentBill && !currentBill.isPaid) {
          totalDue = currentBill.totalDue;
      }

      return { payments: DB.fees.payments[sid] || {}, mandatory, mandatoryPaid: 0, paidAmount, dueAmount: totalDue, totalAmount: paidAmount + totalDue };
    }
...
<div class="g2">
  ...
  <div class="card fu d3" style="padding:0;overflow:hidden;">
    ...
    function calcStudentFeeBill(sid, month) {
      ...
      const tuition = (s.customFee !== undefined && s.customFee !== '') ? num(s.customFee) : monthlyFeeAmount(s);
      const van = num(s.vanFee || 0);
      const arrears = mRow ? num(mRow.arrearsSnapshot || 0) : num(s.arrears || 0);
      
      let itemsTotal = 0;
      let breakdownHtml = `<div style="display:flex;justify-content:space-between;margin-bottom:4px;"><span>Monthly Tuition:</span> <b>${fmtMoney(tuition)}</b></div>`;
      ...
      if (arrears > 0) breakdownHtml += `<div style="display:flex;justify-content:space-between;margin-bottom:4px;color:var(--o);"><span>Previous Arrears:</span> <b>${fmtMoney(arrears)}</b></div>`;
      ...
      const totalDue = tuition + van + arrears + itemsTotal;
    ...
    function receiveStudentPayment(sid, month, totalDue, snapshotArrears) {
      ...
      DB.fees.monthlyLedger[sid][month] = { 
         ...
         arrearsRolledOver: remaining,
         arrearsSnapshot: snapshotArrears
      };
      
      s.arrears = remaining;
      s.fees = remaining === 0 ? 'Paid' : 'Pending';
    ...
    function unmarkStudentPayment(sid, month) {
       const s = allStus().find(x => x.id === sid); if (!s) return;
       if(!confirm('Undo payment? This will revert the student arrears balance.')) return;
       const mRow = DB.fees.monthlyLedger[sid][month];
       if(mRow) {
          s.arrears = mRow.arrearsSnapshot || 0;
